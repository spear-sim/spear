...
class OpenBotPIDPolicy():
    ...
    def __init__(self, config): 
        ...
        assert self.config.IMITATION_LEARNING_OPENBOT.ACCEPTANCE_RADIUS >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.PID.PROPORTIONAL_GAIN_DIST >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.PID.DERIVATIVE_GAIN_DIST >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.PID.PROPORTIONAL_GAIN_HEADING >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.PID.DERIVATIVE_GAIN_HEADING >= 0.0
        assert self.config.IMITATION_LEARNING_OPENBOT.PID.FORWARD_MIN_ANGLE >= 0.0
    ...
    def step(self, obs):
        ...
        lin_vel_norm = np.linalg.norm((obs["state_data"][0:2] - self.position_xy_old) / self.config.SIMULATION_CONTROLLER.SIMULATION_STEP_TIME_SECONDS) * 0.01 
        self.position_xy_old = obs["state_data"][0:2]
        ang_vel_norm = (obs["state_data"][4] - self.yaw_old)/self.config.SIMULATION_CONTROLLER.SIMULATION_STEP_TIME_SECONDS
        self.yaw_old = obs["state_data"][4]

        # compute angular component of motion 
        right_ctrl = -self.config.IMITATION_LEARNING_OPENBOT.PID.PROPORTIONAL_GAIN_HEADING * yaw_error - self.config.IMITATION_LEARNING_OPENBOT.PID.DERIVATIVE_GAIN_HEADING * ang_vel_norm;
        right_ctrl = np.clip(right_ctrl, -self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION, self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION)
        ...
        forward_ctrl = 0.0
        if abs(yaw_error) <= self.config.IMITATION_LEARNING_OPENBOT.PID.FORWARD_MIN_ANGLE:
            forward_ctrl += self.config.IMITATION_LEARNING_OPENBOT.PID.PROPORTIONAL_GAIN_DIST * xy_position_error_norm - self.config.IMITATION_LEARNING_OPENBOT.PID.DERIVATIVE_GAIN_DIST * lin_vel_norm
            forward_ctrl = np.clip(forward_ctrl, -self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION, self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION)
        ...
        left_wheel_command = np.clip(left_wheel_command, -self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION, self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION)
        right_wheel_command = forward_ctrl - right_ctrl
        right_wheel_command = np.clip(right_wheel_command, -self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION, self.config.IMITATION_LEARNING_OPENBOT.CONTROL_SATURATION)
        action = np.array([left_wheel_command,right_wheel_command], dtype=np.float32)
        
        policy_step_info = {}
        policy_step_info["current_waypoint"] = self.trajectory[self.index_waypoint]
        
        # is the current waypoint close enough to be considered as "reached" ?
        waypoint_reached = (xy_position_error_norm <= self.config.IMITATION_LEARNING_OPENBOT.ACCEPTANCE_RADIUS)
        policy_step_info["waypoint_reached"] = waypoint_reached
        ...
        if waypoint_reached: 
            if self.index_waypoint < num_waypoints: # if this waypoint is not the final "goal"
                ...
                policy_step_info["goal_reached"] = False 
            else: # if this waypoint is the final "goal"
                policy_step_info["goal_reached"] = True 
        else:
            policy_step_info["goal_reached"] = False 
            
        return action, policy_step_info
...
class OpenBotPilotNetPolicy():
    ...
    def step(self, obs, position_xy_desired):
        ...
        # is the current waypoint close enough to be considered as "reached" ?
        step_info = {"goal_reached" : (xy_position_error_norm <= self.config.IMITATION_LEARNING_OPENBOT.ACCEPTANCE_RADIUS)}
