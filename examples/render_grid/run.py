...
if __name__ == "__main__":
    ...
    for i in range(rows):
        for j in range(cols):
            # these offsets define the spatial displacement of the cameras in the grid relative to the parent actor’s pose
    ...
    with instance.begin_frame():
        ...
        parent_root_component = game.unreal_service.create_scene_component_by_class_from_actor(
            scene_component_class=scene_component_static_class,
            owner=parent_actor,
            scene_component_name="DefaultSceneRoot")

        attach_to_component_func = game.unreal_service.find_function_by_name(
        ...
        for offset in camera_offsets:
            ...
            # instead of calling our own attach function, use the existing UFUNCTION, retrieve the sensor actor’s root scene component
            child_root_component = game.unreal_service.get_component_by_name(
                class_name="USceneComponent",
                actor=bp_camera_sensor_actor,
        ...
        # for the sake of demonstration, we'll show moving the parent actor will move the entire grid of cameras
        ...
        for (bp_camera_sensor_actor, final_tone_curve_hdr_component) in zip(bp_camera_sensor_actors, final_tone_curve_hdr_components):
    ...
    for frame_idx in range(num_frames):
        with instance.begin_frame():
            ...
            # walk foward along the look-at direction, but don't change the height above the floor
            x = frame_idx

            # add a figure-eight pattern in the camera plane
            y = np.sin(t)
            z = np.sin(t) * np.cos(t)
    ...
    with instance.begin_frame():
        pass
    with instance.end_frame():
        for (bp_camera_sensor_actor, final_tone_curve_hdr_component, final_tone_curve_hdr_component_shared_memory_handles) in \
            zip(bp_camera_sensor_actors, final_tone_curve_hdr_components, final_tone_curve_hdr_component_shared_memory_handles_list):
