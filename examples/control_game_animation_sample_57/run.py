...
if __name__ == "__main__":
    ...
    with instance.begin_frame():
        ...
        # get old and new player controllers
        old_player_controller = gameplay_statics.GetPlayerController(PlayerIndex=0)

        new_player_controller_uclass = game.unreal_service.load_class(uclass="APlayerController", name="/Game/Locomotor/PC_Locomotor.PC_Locomotor_C")
        new_player_controller = game.unreal_service.spawn_actor(uclass=new_player_controller_uclass)

        # swap player controllers
        game_mode = gameplay_statics.GetGameMode()
        sp_game_mode_base = game.get_unreal_object(uclass="USpGameModeBase")
        sp_game_mode_base.SwapPlayerControllers(GameModeBase=game_mode, OldPC=old_player_controller, NewPC=new_player_controller)

        # update player controller
        player_controller = new_player_controller

        # spawn walker
        bp_walker_uclass = game.unreal_service.load_class(uclass="AActor", name="/Game/Locomotor/BP_Walker.BP_Walker_C")
        bp_walker = game.unreal_service.spawn_actor(
            uclass=bp_walker_uclass,
            location={"X": -4200.0, "Y": 5000.0, "Z": 100.0}, 
            rotation={"Pitch": 0.0, "Yaw": 0.0, "Roll": 0.0},
            spawn_parameters={"SpawnCollisionHandlingOverride": "AlwaysSpawn"})
        player_controller.Possess(InPawn=bp_walker)
    ...
    for i in range(90):
        with instance.begin_frame():
            ...
            instance.enhanced_input_service.inject_input_for_actor(
                actor=player_controller, # actions are on the player controller in this case
    ...
    for i in range(170):
        with instance.begin_frame():
            ...
            action_trigger_event = "Triggered"
            action_value_type = "Axis2D"
            action_value = {"X": 0.0, "Y": 1.0, "Z": 0.0}
    ...
    for i in range(30):
        with instance.begin_frame():
            ...
            action_trigger_event = "Triggered"
            action_value_type = "Axis2D"
            action_value = {"X": -0.5, "Y": -0.1, "Z": 0.0}
