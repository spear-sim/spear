...
if __name__ == "__main__":
    ...
    for filesystem_sub_dir in filesystem_sub_dirs[begin_index: end_index+1]:
        ...
        spear.log(f"    Processing filesystem subdirectory: ", filesystem_sub_dir)
        ...
        for fbx_file in fbx_files:
            ...
            max_content_dir_length = (max_content_path_length - content_path_extra_chars - len(args.content_base_dir) - 2*(len("BP_") + len(fbx_file) + len("_Anim") - len(".fbx"))) // num_content_file_parts
            spear.log(f"    Truncating content dirs to {max_content_dir_length} chars to stay within Unreal's limits for content paths...")

            fbx_content_dir = truncate_from_middle(string=replace_chars(string=filesystem_sub_dir, replace_chars_map=replace_chars_map), max_length=max_content_dir_length)
            ...
            fbx_content_path = posixpath.join(args.content_base_dir, fbx_content_dir, fbx_content_name)

            if args.skip_if_exists and unreal.EditorAssetLibrary.does_directory_exist(directory_path=fbx_content_path):
                spear.log("        Directory exists, skipping: ", fbx_content_path)
                continue

            if unreal.EditorAssetLibrary.does_directory_exist(directory_path=fbx_content_path):
                spear.log("        Directory exists, removing: ", fbx_content_path)
                success = unreal.EditorAssetLibrary.delete_directory(directory_path=fbx_content_path)
                assert success
                success = unreal.EditorAssetLibrary.make_directory(directory_path=fbx_content_path)
                assert success
            else:
                spear.log("        Directory doesn't exist: ", fbx_content_path)

            fbx_filesystem_path = os.path.join(args.filesystem_base_dir, filesystem_sub_dir, fbx_file)

            spear.log(f"        Importing {fbx_filesystem_path} to {fbx_content_path}...")
            ...
            asset_import_task.set_editor_property(name="destination_path", value=fbx_content_path)
            ...
            assert unreal.EditorAssetLibrary.does_directory_exist(directory_path=fbx_content_path)
            asset_paths = unreal.EditorAssetLibrary.list_assets(directory_path=fbx_content_path)
            ...
            for asset_path in asset_paths:
                ...
                if asset_data.get_editor_property(name="asset_class_path").get_editor_property(name="asset_name") == "AnimSequence":
                    ...
                    skeletal_mesh_path = posixpath.join(anim_sequence_dir, f"{fbx_content_name}.{fbx_content_name}")
