...
class SimpleEnv(gym.Env):
    def __init__(self, env_config):
        ...
        # """
        # self.action_space Dict(add_force:Box(-1.0, 1.0, (3,), float64), add_to_rotation:Box(-1.0, 1.0, (3,), float64))
        # self.observation_space Dict(camera.final_color:Box(0, 255, (480, 640, 4), uint8), location:Box(-10000.0, 10000.0, (3,), float64), rotation:Box(-10000.0, 10000.0, (3,), float64))
        # """
    ...
    def step(self, action):
        ...
        new_rotation = current_rotation + action['add_to_rotation']
        transform_args = {
            "NewLocation": dict(zip(["X", "Y", "Z"], new_location.tolist())),
            "NewRotation": dict(zip(["Roll", "Pitch", "Yaw"], new_rotation.tolist())),
            "bSweep": False,
        ...
        if done:
            reward += 1000
        if np.linalg.norm(current_location) > 1000:
            done = True
        info = {}
        # info = {'task_step_info': {'hit_goal': np.array([0], dtype=np.uint8), 'hit_obstacle': np.array([0], dtype=np.uint8)},
        #         'agent_step_info': {'debug': np.array([[0., 1., 2.], [3., 4., 5.]])}}
        truncated = False
        # print("obs", obs)
        print("step", new_location, new_rotation, action, reward, done, info)
