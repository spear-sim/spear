...
if __name__ == "__main__":
    ...
    with instance.begin_frame():
        ...
        # spawn agent dynamically in unreal
        urdf_agent = instance.unreal_service.spawn_actor_from_uclass(
            uclass=urdf_uclass,
            location={"X": 0.0, "Y": 100.0, "Z": 30.0}, rotation={"Roll": 0.0, "Pitch": 0.0, "Yaw": 0.0},
            spawn_parameters={"Name": "urdf_agent", "SpawnCollisionHandlingOverride": "AlwaysSpawn"}
        )
        agent_link_components = instance.unreal_service.get_children_components_by_class_as_dict_from_actor(urdf_agent, static_mesh_component_uclass)
    ...
    while mj_viewer.is_running():
        ...
        with instance.begin_frame():
            ...
            # update agent
            for full_link_name, link_component in agent_link_components.items():
                link_name = full_link_name[full_link_name.rfind(".") + 1:]
                if link_name in mj_bodies_xpos:
                    instance.unreal_service.call_function(link_component, set_component_world_location_and_rotation_func, {
                        "NewLocation": dict(zip(["X", "Y", "Z"], mj_bodies_xpos[link_name].tolist())),
                        "NewRotation": dict(zip(["Roll", "Pitch", "Yaw"], unreal_rpy_from_mujoco_quaternion(mj_bodies_xquat[link_name]).tolist())),
                        "bSweep": False,
                        "bTeleport": True})
