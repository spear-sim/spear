...
You should see a game window appear, some console output indicating rendering progress, you should see an image saved in the same directory as this `README` file. This example is only expected to run on platforms where Unreal supports Hardware Ray Tracing. See [here](https://dev.epicgames.com/documentation/unreal-engine/hardware-ray-tracing-in-unreal-engine) for details.
