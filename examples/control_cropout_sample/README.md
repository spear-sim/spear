...
In a pre-processing step, we need to install the SPEAR plugins into the `CropoutSample` project. We must also build the project, which will build the SPEAR plugin binaries and copy them into the project directory.

```console
python ../../tools/install_plugins_in_external_project.py --external-project-dir path/to/CropoutSampleProject

python ../../tools/run_uat.py --unreal-engine-dir path/to/UE_5.8 --unreal-project-dir path/to/CropoutSampleProject -build
```
