...
In a pre-processing step, we need to install the SPEAR plugins into the `ElectricDreams` project. We must also build the project, which will build the SPEAR plugin binaries and copy them into the project directory.

```console
python ../../tools/install_plugins_in_external_project.py --external-project-dir path/to/ElectricDreamsEnv

python ../../tools/run_build.py --unreal-engine-dir path/to/UE_5.8 --unreal-project-dir path/to/ElectricDreamsEnv --build-target ElectricDreamsSampleEditor
```
