...
_Figure: The SPEAR Python library can call any C++ function, and can access any C++ variable, on any game entity, and any game subsystem, in any Unreal Engine application, provided the function or variable has been exposed to Unreal's visual scripting system, i.e., Blueprints. We demonstrate this unique capability by controlling 4 embodied agents, each with a distinct action space, in several sample projects that are available from Epic Games. Controlling these agents (i.e., a person from `CitySample` - top left, a car from `CitySample` - top right, a flying robot from `StackOBot` - bottom right, and a person with parkour skills from `GameAnimationSample` - bottom right) using SPEAR only requires adding a single line to each project definition file. Otherwise the projects can remain completely unmodified, while being comprehensively controllable from Python via SPEAR._
