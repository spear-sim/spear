...
_Figure: The SPEAR Python library can call any C++ function, and can access any C++ variable, on any game entity, and any game subsystem, provided the function or variable has been exposed to Unreal's visual scripting system, i.e., Blueprints. We demonstrate this unique capability by controlling 4 distinct embodied agents, each with a distinct action space, in the `CitySample` (top left and top right), `StackOBot` (bottom left), and `GameAnimationSample` (bottom right) sample projects that are freely available from Epic Games. Controlling these agents using SPEAR only requires adding a single line to each project definition file. Otherwise the projects are completely unmodified. We provide all the necessary code to control these agents and render these images in our `examples` directory._
