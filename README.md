...
_Figure: The SPEAR Python library can call any C++ function, and can access any C++ variable, on any game entity, and any game subsystem, in any Unreal Engine application, provided the function or variable has been exposed to Unreal's visual scripting system, i.e., Blueprints. We demonstrate this unique capability by controlling 5 embodied agents, each with a distinct action space, in several sample projects that are freely available from Epic Games: a person and a car from `CitySample` (top); a flying robot from `StackOBot` (bottom left); a person with parkour skills from `GameAnimationSample` (bottom middle); and multiple agents from the `CropoutSample` farming resource management game (bottom right). SPEAR can control the agents in these projects, and perform customizable high-speed rendering, without requiring any modifications to SPEAR library code or the projects themselves, aside from adding a single-line declaration to each project definition file. No other changes are required to enable comprehensive programmatic control of complex in-the-wild Unreal projects from Python via SPEAR._
