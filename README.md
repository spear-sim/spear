...
_Figure: The SPEAR Python library can call any C++ function, and can access any C++ variable, on any game entity, and any game subsystem, provided the function or variable has been exposed to Unreal's visual scripting system, i.e., Blueprints. We demonstrate this unique capability by controlling 4 embodied agents, each with a distinct action space, in sample projects that are freely available from Epic Games. Controlling these agents (i.e., a person from `CitySample` shown in the top left, a car from `CitySample` shown in the top right, a flying robot from `StackOBot` shown in the bottom right, and a person with parkour skills from `GameAnimationSample` shown in the bottom right) using SPEAR only requires adding a single line to each project definition file. Otherwise the projects can remain completely unmodified. In this repository, we provide all the necessary code to control each agent and render each image above._
