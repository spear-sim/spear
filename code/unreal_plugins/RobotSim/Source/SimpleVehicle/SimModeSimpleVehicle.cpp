...
void ASimModeSimpleVehicle::BeginPlay()
{
    ...
    Super::BeginPlay();
    URobotBlueprintLib::BindActionToKey("test", EKeys::One, this, &ASimModeSimpleVehicle::setupVehiclesAndCamera);
    URobotBlueprintLib::BindActionToKey("test2", EKeys::Two, this, &ASimModeSimpleVehicle::DestroyPawn);
    URobotBlueprintLib::BindActionToKey("inputEventCycleCameraForward", EKeys::N, this, &ASimModeSimpleVehicle::cycleVisibleCameraForward);
    URobotBlueprintLib::BindActionToKey("inputEventCycleCameraBackward", EKeys::P, this, &ASimModeSimpleVehicle::cycleVisibleCameraBackward);

    URobotBlueprintLib::BindActionToKey("test", EKeys::Three, this, &ASimModeSimpleVehicle::Test);
}

void ASimModeSimpleVehicle::getExistingVehiclePawns(TArray<RobotBase*>& pawns) const
{
...
void ASimModeSimpleVehicle::setupVehiclesAndCamera()
{
    ...
    if (pawns.Num() > 0)
    {
        UE_LOG(LogTemp, Warning, TEXT("ASimModeSimpleVehicle::setupVehiclesAndCamera pawns.length()>0"));
        return;
    }
    ...
    for (auto const& vehicle_setting_pair : this->getSettings().vehicles)
    {
        ...
        std::string vehicle_name = vehicle_setting_pair.first + "_" + std::to_string(this->reset_count);
        const auto& vehicle_setting = *vehicle_setting_pair.second;

        if (vehicle_setting.auto_create and vehicle_setting.vehicle_type == RobotSimSettings::kVehicleTypeSimpleVehicle)
        {
            // Spawn setting
            ...
            std::string PawnBP = getSettings().pawn_paths.at("SimpleVehicle").pawn_bp;
            ...
            {
                // Random Rz
                spawnRotation = spawnRotation + FRotator(0, FMath::FRandRange(0, 360), 0);
            ...
            if (vehicle_setting.enable_spawn_tracing_ground)
            {
                ...
                traceGround(spawnPosition, spawnRotation, bound.GetBox().GetSize() / 2);
                spawnPosition = spawnPosition + FVector(0, 0, -3);
            }
            // URobotBlueprintLib::LogMessage("spawn at pos", spawnPosition.ToString() + " ori: " + spawnRotation.Euler().ToString(), LogDebugLevel::Informational, 30);
    ...
    for (RobotBase* pawn : pawns)
    {
        ...
        ASimpleVehiclePawn* vehicle_pawn = static_cast<ASimpleVehiclePawn*>(pawn);
        ...
        params.collision_display_template = collision_display_template; // Collision info
        params.home_geopoint = home_geopoint;
        params.vehicle_name = "SimpleVehicle"; // TODO: This may need to be changed for multiple vehicles.

        auto vehicle_sim_api = std::unique_ptr<SimpleVehicleSimApi>(new SimpleVehicleSimApi(params));
...
void ASimModeSimpleVehicle::DestroyPawn()
{
    ...
    for (auto& pawn : pawns)
    {
        ASimpleVehiclePawn* simpleVehiclePawn = static_cast<ASimpleVehiclePawn*>(pawn);
