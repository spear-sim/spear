...
private:
    ...
    Eigen::Vector4f
        motorVelocity_; // The ground truth velocity of the motors in [rad/s]

    Eigen::Vector4f
        counterElectromotiveForce_; // Vector of counter electromotive forces of
                                    // each motor [V]

    Eigen::Vector4f motorTorque_; // The torque generated by the motors in [N.m]

    Eigen::Vector4f
        wheelTorque_; // The torque applied to the robot wheels in [N.m]

    Eigen::Vector4f dutyCycle_; // The duty cycle of the PWM signal applied to
                                // the motors in [%]

    Eigen::Vector4f motorWindingCurrent_; // Electrical current circulating into
                                          // the DC motor windings in [A]

    Eigen::Vector2f actionVec_; // Vector of action observations to be fed back
                                // to the RL algorithm.
    ...
    float motorTorqueMax_ = 0.1177f; // Maximum ammount of torque an OpenBot
                                     // motor can generate [N.m].
    ...
    float actionScale_ =
        255.f; // Speed multiplier defined in the OpenBot to
               // map a [-1,1] action to a suitable command to
               // be processed by the low-level
               // microcontroller. For more details, feel free
               // to check the "speedMultiplier" command in the
               // OpenBot code:
    ...
    float electricalResistance_ = 4.2f; // Electrical resistance of the DC motor
                                        // windings in [Ohms]

    float electricalInductance_ = 0.f; // Electrical inductance of the DC motor
                                       // windings in [Henry]

    float controlDeadZone_ =
        5.0; // Below this command threshold, the torque is set to
