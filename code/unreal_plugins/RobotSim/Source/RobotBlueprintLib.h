...
public:
    ...
    static T* FindActor(const UObject* context, FString name)
    {
        ...
        {
            if (actor->ActorHasTag(name_n) ||
                actor->GetName().Compare(name) == 0)
            {
                return static_cast<T*>(actor);
            }
        }

        // URobotBlueprintLib::LogMessage(name + TEXT(" Actor not found!"),
    ...
    template <typename T>
    static void FindAllActor(const UObject* context,
                             TArray<AActor*>& foundActors)
    {
    ...
    static bool GetLastObstaclePosition(
        const AActor* actor,
        const FVector& start,
        const FVector& end,
        FHitResult& hit,
    ...
    template <class T> static std::string GetMeshName(T* mesh)
    {
        ...
        {
        case RobotSim::RobotSimSettings::SegmentationSetting::
            ...
            if (mesh->GetOwner())
            {
                ...
                if (owner->GetName().StartsWith("InstancedFoliageActor"))
                {
                    ...
                    {
                        meshName = meshName.Replace(TEXT("IFA_"), TEXT(""))
                                       .Replace(TEXT("_C_"), TEXT(""));
                    }
        ...
        case RobotSim::RobotSimSettings::SegmentationSetting::
            MeshNamingMethodType::StaticMeshName:
            if (mesh)
    ...
    static FInputActionBinding& BindActionToKey(
        const FName action_name,
        const FKey in_key,
        UserClass* actor,
        typename FInputActionHandlerSignature::TUObjectMethodDelegate<
            UserClass>::FMethodPtr func,
    ...
    {
        FInputActionKeyMapping action(action_name, in_key, shift_key,
                                      control_key, alt_key, command_key);

        APlayerController* controller =
            actor->GetWorld()->GetFirstPlayerController();

        controller->PlayerInput->AddActionMapping(action);
        return controller->InputComponent->BindAction(
            action_name, on_press_or_release ? IE_Pressed : IE_Released, actor,
            func);
    }

    template <class UserClass>
    static FInputAxisBinding&
    BindAxisToKey(const FName axis_name,
                  const FKey in_key,
                  AActor* actor,
                  UserClass* obj,
                  typename FInputAxisHandlerSignature::TUObjectMethodDelegate<
                      UserClass>::FMethodPtr func)
    {
    ...
    template <class UserClass>
    static FInputAxisBinding&
    BindAxisToKey(const FInputAxisKeyMapping& axis,
                  AActor* actor,
                  UserClass* obj,
                  typename FInputAxisHandlerSignature::TUObjectMethodDelegate<
                      UserClass>::FMethodPtr func)
    ...
    static void SetMeshNamingMethod(
    ...
    static std::vector<UPrimitiveComponent*>
    getPhysicsComponents(AActor* actor);
...
private:
    ...
    static void InitializeObjectStencilID(T* mesh, bool ignore_existing = true)
    {
        ...
        if (mesh_name == "" ||
            common_utils::Utils::startsWith(mesh_name, "default_"))
        {
        ...
        {
            auto char_num =
                UKismetStringLibrary::GetCharacterAsNumber(name, idx);
            if (char_num < 97)
                continue; // numerics and other punctuations
            hash += char_num;
        }
        if (ignore_existing || mesh->CustomDepthStencilValue == 0)
        { // if value is already set then don't bother
                                          ...
                                          int& changes)
    {
        ...
        bool is_match =
            (!is_name_regex && (comp_mesh_name == mesh_name ||
                                comp_mesh_name == (mesh_name + "_visual"))) ||
...
private:
    ...
    // FViewPort doesn't expose this field so we are doing dirty work around by
    // maintaining count by ourselves
    static uint32_t flush_on_draw_count_;
    static RobotSim::RobotSimSettings::SegmentationSetting::MeshNamingMethodType
        mesh_naming_method_;
