...
void APIPCamera::SetupCaptureComponent()
{
    ...
    renderTarget2D->TargetGamma = GEngine->GetDisplayGamma(); // 1.2f; // for Vulkan
                                 // //GEngine->GetDisplayGamma(); // for DX11/12

    // Setup the RenderTarget capture format
    renderTarget2D->InitAutoFormat(512, 512); // some random format, got crashing otherwise
    // int32 frameWidht = 2048;
    // int32 frameHeight = 2048;
    renderTarget2D->InitCustomFormat(512, 512, PF_B8G8R8A8,true); // PF_B8G8R8A8 disables HDR which will boost storing to disk due
