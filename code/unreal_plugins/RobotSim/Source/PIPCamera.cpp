...
{
    static ConstructorHelpers::FObjectFinder<UMaterial> mat_finder(TEXT(
        "Material'/RobotSim/HUDAssets/CameraSensorNoise.CameraSensorNoise'"));
...
void APIPCamera::PostInitializeComponents()
{
    ...
    camera_ = URobotBlueprintLib::GetActorComponent<UCameraComponent>(
    ...
    captures_[Utils::toNumeric(ImageType::Scene)] =
        URobotBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(
            this, TEXT("SceneCaptureComponent"));
    captures_[Utils::toNumeric(ImageType::DepthPlanner)] =
        URobotBlueprintLib::GetActorComponent<USceneCaptureComponent2D>(
            this, TEXT("DepthPlannerCaptureComponent"));
...
APIPCamera::getProjectionMatrix(const APIPCamera::ImageType image_type) const
{
    ...
    if (capture)
    {
        ...
        FIntPoint render_target_size(
            capture->TextureTarget->GetSurfaceWidth(),
        ...
        if (render_target_size.X > render_target_size.Y)
        {
            ...
            y_axis_multiplier =
                render_target_size.X / static_cast<float>(render_target_size.Y);
        }
        else
        {
        ...
        if (capture->ProjectionType == ECameraProjectionMode::Orthographic)
        {
            ...
            const float OrthoHeight = capture->OrthoWidth / 2.0f *
                                      x_axis_multiplier / y_axis_multiplier;
        ...
        else
        {
            ...
            if ((int32)ERHIZBuffer::IsInverted)
            {
                proj_mat = FReversedZPerspectiveMatrix(
                    fov, fov, x_axis_multiplier, y_axis_multiplier,
...
void APIPCamera::showToScreen()
{
    ...
    URobotBlueprintLib::LogMessage(TEXT("Camera: "), GetName(),
    ...
    const NedTransform& ned_transform)
{
    ...
    for (int image_type = -1; image_type < image_count; ++image_type)
    {
        ...
        const auto& noise_setting =
            camera_setting.noise_settings.at(image_type);

        if (image_type >= 0)
        { // scene capture components
            updateCaptureComponentSetting(captures_[image_type],
                                          render_targets_[image_type],
...
void APIPCamera::updateCameraSetting(UCameraComponent* camera,
                                     const CaptureSetting& setting,
                                     const NedTransform& ned_transform)
{
    // if (!std::isnan(setting.target_gamma))
...
void APIPCamera::setNoiseMaterial(int image_type,
                                  UObject* outer,
                                  ...
                                  const NoiseSetting& settings)
{
    ...
    UMaterialInstanceDynamic* noise_material_ =
    ...
    noise_material_->SetScalarParameterValue("RandDensity",
                                             settings.RandDensity);
    noise_material_->SetScalarParameterValue("RandContrib",
                                             settings.RandContrib);
    noise_material_->SetScalarParameterValue("HorzWaveContrib",
                                             settings.HorzWaveContrib);
    noise_material_->SetScalarParameterValue("HorzWaveVertSize",
                                             settings.HorzWaveVertSize);
    noise_material_->SetScalarParameterValue("HorzWaveScreenSize",
                                             settings.HorzWaveScreenSize);
    noise_material_->SetScalarParameterValue("HorzNoiseLinesContrib",
                                             settings.HorzNoiseLinesContrib);
    noise_material_->SetScalarParameterValue("HorzNoiseLinesDensityY",
                                             settings.HorzNoiseLinesDensityY);
    noise_material_->SetScalarParameterValue("HorzNoiseLinesDensityXY",
                                             settings.HorzNoiseLinesDensityXY);
    noise_material_->SetScalarParameterValue("HorzDistortionStrength",
                                             settings.HorzDistortionStrength);
    noise_material_->SetScalarParameterValue("HorzDistortionContrib",
                                        ...
                                        bool is_enabled)
{
    ...
    if (capture != nullptr)
    {
        if (is_enabled)
        {
            // do not make unnecessary calls to Activate() which otherwise
            // causes crash in Unreal
