...
void ACameraDirector::Tick(float DeltaTime)
{
    ...
    { // make camera move in desired way
        URobotBlueprintLib::FollowActor(ExternalCamera, follow_actor_,
                                             ...
                                             APIPCamera* back_camera)
{
    manual_pose_controller_ = NewObject<UManualPoseController>(
        this, "CameraDirector_ManualPoseController");
    ...
    initial_ground_obs_offset_ =
        camera_start_location_ - (follow_actor_
                                      ? follow_actor_->GetActorLocation()
                                      : FVector::ZeroVector);
    ...
    case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_FRONT:
        ...
        break;
    default:
        throw std::out_of_range("Unsupported view mode specified in "
...
void ACameraDirector::attachSpringArm(bool attach)
{
    if (attach)
    {
        // If we do have actor to follow AND don't have sprint arm attached to
        // that actor, we will attach it
        if (follow_actor_ &&
            ExternalCamera->GetRootComponent()->GetAttachParent() != SpringArm)
        {
            // For car, we want a bit of camera lag, as that is customary of
            // racing video games If the lag is missing, the camera will also
            // occasionally shake. But, lag is not desired when piloting a drone
            SpringArm->bEnableCameraRotationLag = camera_rotation_lag_enabled_;

            // attach spring arm to actor
            SpringArm->AttachToComponent(
                follow_actor_->GetRootComponent(),
                FAttachmentTransformRules::KeepRelativeTransform);
            SpringArm->SetRelativeLocation(FVector(0.0f, 0.0f, 34.0f));

            // remember current parent for external camera. Later when we remove
            // external camera from spring arm, we will attach it back to its
            // last parent
            last_parent_ =
                ExternalCamera->GetRootComponent()->GetAttachParent();
            ExternalCamera->DetachFromActor(
                FDetachmentTransformRules::KeepWorldTransform);
            // now attach camera to spring arm
            ExternalCamera->AttachToComponent(
                SpringArm, FAttachmentTransformRules::KeepRelativeTransform);
        }

        // For car, we need to move the camera back a little more than for a
        // drone. Otherwise, the camera will be stuck inside the car
        ExternalCamera->SetActorRelativeLocation(
            FVector(follow_distance_ * 100.0f, 0.0f, 0.0f));
    ...
    { // detach
            ...
            ExternalCamera->GetRootComponent()->GetAttachParent() == SpringArm)
        {
            ...
            ExternalCamera->AttachToComponent(
                last_parent_, FAttachmentTransformRules::KeepRelativeTransform);
        }
    }
}
...
void ACameraDirector::setMode(ECameraDirectorMode mode)
{
    { // first remove any settings done by previous mode
        ...
        // detach spring arm
        if (mode_ ==
                ECameraDirectorMode::CAMERA_DIRECTOR_MODE_SPRINGARM_CHASE &&
        ...
        {
            URobotBlueprintLib::enableViewportRendering(this, true);
        }
        ...
        if (mode != ECameraDirectorMode::CAMERA_DIRECTOR_MODE_MANUAL)
        {
            ...
            // else someone else is bound to manual pose controller, leave it
            // alone
        }
    }
    ...
    { // perform any settings to enter in to this mode
        ...
        case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_MANUAL:
            ...
            break;
        case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_SPRINGARM_CHASE:
            // if we switched to spring arm mode then attach to spring arm
            // (detachment was done earlier in method)
            attachSpringArm(true);
            break;
        case ECameraDirectorMode::CAMERA_DIRECTOR_MODE_NODISPLAY:
            URobotBlueprintLib::enableViewportRendering(this, false);
            break;
        default:
...
void ACameraDirector::setupInputBindings()
{
    ...
    // URobotBlueprintLib::BindActionToKey("inputEventFpvView", EKeys::F, this,
    // &ACameraDirector::inputEventFpvView);
    // URobotBlueprintLib::BindActionToKey("inputEventFlyWithView", EKeys::B,
    // this, &ACameraDirector::inputEventFlyWithView);
    // URobotBlueprintLib::BindActionToKey("inputEventGroundView",
    // EKeys::Backslash, this, &ACameraDirector::inputEventGroundView);
    // URobotBlueprintLib::BindActionToKey("inputEventManualView", EKeys::M,
    // this, &ACameraDirector::inputEventManualView);
    // URobotBlueprintLib::BindActionToKey("inputEventSpringArmChaseView",
    // EKeys::Slash, this, &ACameraDirector::inputEventSpringArmChaseView);
    // URobotBlueprintLib::BindActionToKey("inputEventBackupView", EKeys::K,
    // this, &ACameraDirector::inputEventBackupView);
    // URobotBlueprintLib::BindActionToKey("inputEventNoDisplayView",
    // EKeys::Hyphen, this, &ACameraDirector::inputEventNoDisplayView);
    // URobotBlueprintLib::BindActionToKey("inputEventFrontView", EKeys::I,
    // this, &ACameraDirector::inputEventFrontView);
}
...
void ACameraDirector::inputEventSpringArmChaseView()
{
    if (ExternalCamera)
    {
        ...
        URobotBlueprintLib::LogMessageString(
            "Camera is not available: ", "ExternalCamera",
...
void ACameraDirector::inputEventBackupView()
{
    if (backup_camera_)
    {
        ...
        URobotBlueprintLib::LogMessageString(
            "Camera is not available: ", "backup_camera",
            LogDebugLevel::Failure);
}
    ...
    if (ExternalCamera)
    {
        ...
        if (follow_actor_)
            ExternalCamera->SetActorLocationAndRotation(
                follow_actor_->GetActorLocation() + initial_ground_obs_offset_,
...
void ACameraDirector::inputEventFpvView()
{
    if (fpv_camera_)
    {
        ...
        URobotBlueprintLib::LogMessageString(
            "Camera is not available: ", "fpv_camera", LogDebugLevel::Failure);
}

void ACameraDirector::disableCameras(bool fpv,
                                     bool backup,
                                     bool external,
                                     bool front)
{
