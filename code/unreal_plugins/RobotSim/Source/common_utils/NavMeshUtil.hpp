...
struct NavMeshUtil
{
    ...
    static float FindArea(TArray<FVector>& vertices)
    {
        ...
        if (vertices.Num() >= 3)
        {
            ...
            {
                area += 0.5f * FVector::CrossProduct(vertices[i - 1] - start,
                                                     vertices[i] - start)
                                   .Size();
            }
        }
                ...
                bool FilterFlat = true)
    {
        ...
        for (auto& kvp : clusterMap)
        {
            ...
            for (auto PolyId : kvp.Value)
            {
                ...
                if (navMesh->GetPolyVerts(PolyId, OutVerts) &&
                    (!FilterFlat || IsFlat(center, OutVerts)))
                {
                                                 ...
                                                 FVector& spawnLocation)
    {
        ...
        TMap<NavNodeRef, float> areaMap;
        NavNodeRef clusterIdwithMaxArea =
            RobotSim::NavMeshUtil::FindAreaMap(navMesh, clusterMap, areaMap);
        FVector startLoc;
