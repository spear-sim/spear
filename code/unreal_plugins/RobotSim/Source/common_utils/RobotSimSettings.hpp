...
namespace RobotSim {
    ...
    public:                                       // types
        ...
        struct Rotation {
            ...
            bool hasNan()
            {
                std::isnan(yaw) or std::isnan(pitch) or std::isnan(roll);
            }
        ...
        struct ActuationSetting {
            ...
            float motorVelocityConstant = 1.0f; // Motor torque constant in [rad/s/V]
            float controlDeadZone = 1.0f;       // Below this command threshold, the torque is set to
                                                // zero if the motor velocity is "small enough"
            float motorTorqueMax = 1.0f;        // Maximum torque a motor (+ gearbox system)
            ...
            float actionScale = 1.0f;           // Actions generated by a neural network or a RL algorithm
    ...
    public: // methods
        ...
        const VehicleSetting* getVehicleSetting(const std::string& vehicle_name) const
        {
            ...
            if (it == vehicles.end()) {
                throw std::invalid_argument(Utils::stringf("VehicleSetting for vehicle name %s was requested but not found", vehicle_name.c_str()).c_str());
            }
            else {
    ...
    private:
            ...
            if (upgrade_required) {
                ...
                if (has_default_settings) {
                    ...
                    if (settings_version_actual == 1) {
                        const std::vector<std::string> all_changed_keys = {"AdditionalCameras", "CaptureSettings", "NoiseSettings", "UsageScenario", "SimpleFlight", "PX4"};
                        ...
                        if (detected_keys.length()) {
                            std::string error_message = "You are using newer version of RobotSim with older version of settings.json. ";
                ...
                if (auto_upgrade) {
                    warning_messages.push_back("You are using newer version of RobotSim with older version of settings.json."
        ...
        bool hasDefaultSettings(const Settings& settings_json, float& version)
        {
            ...
            bool has_docs = (settings_json.getString("SeeDocsAt", "") != "") or (settings_json.getString("see_docs_at", "") != "");
            ...
            has_default |= settings_json.size() == 1 and has_docs;

            // if we have V1 settings and only elements are docs link and version
            has_default |= settings_json.size() == 2 and has_docs and version > 0;
         ...
         */
        static std::unique_ptr<VehicleSetting> createVehicleSetting(const std::string& simmode_name,
                                                                    ...
                                                                    const std::string vehicle_name)
        {
            ...
            if (vehicle_type == kVehicleTypeUrdfBot) {
                ...
                vehicle_setting->controlSetting = createControlSetting(controlSetting);
            }

            // Gear transmission setting:
            Settings actuationSetting;
            settings_json.getChild("ActuationParameters", actuationSetting);
            createActuationSetting(actuationSetting, vehicle_setting->actuationSetting);
            ...
            vehicle_setting->enable_collision_passthrough = settings_json.getBool("EnableCollisionPassthrogh", vehicle_setting->enable_collision_passthrough);
            vehicle_setting->enable_trace = settings_json.getBool("EnableTrace", vehicle_setting->enable_trace);
            vehicle_setting->enable_collisions = settings_json.getBool("EnableCollisions", vehicle_setting->enable_collisions);
            ...
            vehicle_setting->position = createVectorSetting(positionSetting, vehicle_setting->position);
            vehicle_setting->rotation = createRotationSetting(positionSetting, vehicle_setting->rotation);
            vehicle_setting->enable_spawn_tracing_ground = settings_json.getBool("EnableSpawnTracingGround", vehicle_setting->enable_spawn_tracing_ground);
            vehicle_setting->enable_random_spawn = settings_json.getBool("EnableRandomSpawn", vehicle_setting->enable_random_spawn);

            Settings boundingBoxSetting;
            settings_json.getChild("BoundingBox", boundingBoxSetting);
            vehicle_setting->bounding_box = createVectorSetting(boundingBoxSetting, vehicle_setting->bounding_box);

            Settings boundingBoxOffsetSetting;
            settings_json.getChild("BoundingBoxOffset", boundingBoxOffsetSetting);
            vehicle_setting->bounding_box_offset = createVectorSetting(boundingBoxOffsetSetting, vehicle_setting->bounding_box_offset);
                                ...
                                SegmentationSetting& segmentation_setting)
        {
            ...
            if (settings_json.getChild("SegmentationSettings", json_parent)) {
                ...
                if (init_method == "" or init_method == "commonobjectsrandomids") {
        ...
        static ControlSetting createControlSetting(const Settings& settings_json)
        {
            ...
            setting.RightWheelJoint = settings_json.getString("RightWheelJoint", "");
            setting.EndEffectorLink = settings_json.getString("EndEffectorLink", "");
            setting.ManipulatorJoints = settings_json.getStringArray("ManipulatorJoints");
        ...
        {
            actuation_setting.gearRatio = settings_json.getFloat("GearRatio", actuation_setting.gearRatio);
            actuation_setting.motorVelocityConstant = settings_json.getFloat("MotorVelocityConstant", actuation_setting.motorVelocityConstant);
            actuation_setting.controlDeadZone = settings_json.getFloat("ControlDeadZone", actuation_setting.controlDeadZone);
            actuation_setting.motorTorqueMax = settings_json.getFloat("MaximumMotorTorque", actuation_setting.motorTorqueMax);
            actuation_setting.electricalResistance = settings_json.getFloat("WindingResistance", actuation_setting.electricalResistance);
            actuation_setting.electricalInductance = settings_json.getFloat("WindingInductance", actuation_setting.electricalInductance);
            actuation_setting.actionScale = settings_json.getFloat("ActionScaling", actuation_setting.actionScale);
            actuation_setting.batteryVoltage = settings_json.getFloat("BatteryVoltage", actuation_setting.batteryVoltage);
        }
                                          ...
                                          CaptureSetting& capture_setting)
        {
            ...
            capture_setting.target_gamma = settings_json.getFloat("TargetGamma", capture_setting.image_type == 0 ? CaptureSetting::kSceneTargetGamma : Utils::nan<float>());
            ...
            if (projection_mode == "" || projection_mode == "perspective") {
                ...
                throw std::invalid_argument(std::string("CaptureSettings projection_mode has invalid value "
                                                        "in settings_json ") +
                                            projection_mode);
            }
