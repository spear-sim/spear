...
{
    static ConstructorHelpers::FClassFinder<APIPCamera> external_camera_class(TEXT("Blueprint'/RobotSim/Blueprints/BP_PIPCamera'"));
    external_camera_class_ = external_camera_class.Succeeded() ? external_camera_class.Class : nullptr;
    static ConstructorHelpers::FClassFinder<ACameraDirector> camera_director_class(TEXT("Blueprint'/RobotSim/Blueprints/BP_CameraDirector'"));
    camera_director_class_ = camera_director_class.Succeeded() ? camera_director_class.Class : nullptr;

    static ConstructorHelpers::FObjectFinder<UParticleSystem> collision_display(TEXT("ParticleSystem'/RobotSim/StarterContent/Particles/P_Explosion.P_Explosion'"));
    if (!collision_display.Succeeded())
        collision_display_template = collision_display.Object;
    else
        collision_display_template = nullptr;

    static ConstructorHelpers::FClassFinder<APIPCamera> pip_camera_class_val(TEXT("Blueprint'/RobotSim/Blueprints/BP_PIPCamera'"));
...
//                URobotBlueprintLib::LogMessage("Vehicle %s was not initialized: ", 
//                    "", LogDebugLevel::Failure); //TODO: add vehicle name in message
//                URobotBlueprintLib::LogMessage("Tip: check connection info in settings.json", "", LogDebugLevel::Informational);
//            }
//        }
//
//    }
...
void ASimModeBase::setupTimeOfDay()
{
    ...
    if (tod_setting.enabled) {
        ...
        if (sky_spheres.Num() == 0)
            URobotBlueprintLib::LogMessage(TEXT("BP_Sky_Sphere was not found. "), 
                TEXT("TimeOfDay settings would be ignored."), LogDebugLevel::Failure);
        else if (sky_spheres.Num() > 1)
            URobotBlueprintLib::LogMessage(TEXT("More than BP_Sky_Sphere were found. "), 
...
//std::unique_ptr<RobotSim::ApiServerBase> ASimModeBase::createApiServer() const
//{
...
//        ClockFactory::get(std::make_shared<RobotSim::ScalableClock>(clock_speed == 1 ? 1 : 1 / clock_speed));
//    else if (clock_type == "SteppableClock")
//        ClockFactory::get(std::make_shared<RobotSim::SteppableClock>(
//            static_cast<RobotSim::TTimeDelta>(RobotSim::SteppableClock::DefaultStepSize * clock_speed)));
//    else
...
//
//            URobotBlueprintLib::LogMessageString("DateTime: ", Utils::to_string(cur_time), LogDebugLevel::Informational);
//
//            auto coord = RobotSim::EarthCelestial::getSunCoordinates(cur_time, settings.origin_geopoint.home_geo_point.latitude,
...
//            URobotBlueprintLib::LogMessageString("Cannot start RpcLib Server", ex.what(), LogDebugLevel::Failure);
//        }
//    }
//    else
//        URobotBlueprintLib::LogMessageString("API server is disabled in settings", "", LogDebugLevel::Informational);
//
//}
...
//void ASimModeBase::updateDebugReport(RobotSim::StateReporterWrapper& debug_reporter)
//{
...
//
//            const RobotSim::Kinematics::State* kinematics = vehicle_sim_api->getGroundTruthKinematics();
//
...
//FRotator ASimModeBase::toFRotator(const RobotSim::RobotSimSettings::Rotation& rotation, const FRotator& default_val)
//{
...
void ASimModeBase::setupVehiclesAndCamera()
{
    ...
    //            RobotSimVehicle* spawned_pawn = dynamic_cast<RobotSimVehicle*>(spawned_actor);
    ...
    //    {
    //        RobotSimVehicle* vehicle_pawn = static_cast<RobotSimVehicle*>(pawn);
    ...
    //        const auto& home_geopoint= RobotSim::EarthUtils::nedToGeodetic(pawn_ned_pos, getSettings().origin_geopoint);
...
void ASimModeBase::getExistingVehiclePawns(TArray<RobotSimVehicle*>& pawns) const
{
...
//std::string ASimModeBase::getVehiclePawnPathName(const RobotSimSettings::VehicleSetting& vehicle_setting) const
//{
//    //derived class should override this method to retrieve types of pawns they support
//    return "";
//}
...
//}
//RobotSim::VehicleApiBase* ASimModeBase::getVehicleApi(const PawnSimApi::Params& pawn_sim_api_params,
...
//
//        RobotSim::VehicleApiBase* api = getApiProvider()->getVehicleApi(vehicle_name);
//        if (api != nullptr) {
//            
//            RobotSim::uint count_lidars = api->getSensors().size(RobotSim::SensorBase::SensorType::Lidar);
//
//            for (RobotSim::uint i = 0; i < count_lidars; i++) {
//                // TODO: Is it incorrect to assume LidarSimple here?
//                const RobotSim::LidarSimple* lidar =
//                    static_cast<const RobotSim::LidarSimple*>(api->getSensors().getByType(RobotSim::SensorBase::SensorType::Lidar, i));
...
//                        RobotSim::Vector3r point(lidar_data.point_cloud[i], lidar_data.point_cloud[i + 1], lidar_data.point_cloud[i + 2]);
//
