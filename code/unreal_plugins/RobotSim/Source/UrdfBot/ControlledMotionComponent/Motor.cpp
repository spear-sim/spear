...
void Motor::Attach(AUrdfLink* baseLink, AUrdfLink* actuationLink, UrdfJointSpecification *jointSpecification, UPhysicsConstraintComponent* constraintComponent)
{
	...
	float damping = constraintComponent_->ConstraintInstance.ProfileInstance.AngularDrive.TwistDrive.Damping;
	float maxForce = constraintComponent_->ConstraintInstance.ProfileInstance.AngularDrive.TwistDrive.MaxForce;

    if (jointSpecification->Name == "l_wheel_joint" or jointSpecification->Name == "r_wheel_joint")
    {
...
void Motor::SetMotionSpeed(const float& liner_speed)
{
	...
	{
		throw std::runtime_error("the motor radius value is not initialized, please set a reasonable number");
	}
...
float Motor::GetMotionSpeed() const
{
	...
	FVector baseAngularVelocity = this->baseLink_->GetPhysicsAngularVelocityInRadians();
	//relative angular velocity
	FVector worldDiffAngularVelocity = targetAngularVelocity - baseAngularVelocity;
	//angular velocity in base_link frame
	FVector baseFrameAngularVelocity = this->baseLink_->GetTransform().InverseTransformVector(worldDiffAngularVelocity);
	//velocity vector magnitude in axis direction
	float val = FVector::DotProduct(this->jointSpecification_->Axis, baseFrameAngularVelocity);
...
void Motor::ControlRelativeJointPose(TMap<FString, float> controlSignals)
{
    ...
    float delta = controlSignals[TEXT("Value")];	
	if(delta < 0 || delta > 1){
		throw std::runtime_error("Motor control signal has value is outside the expected range of 0 to 1.");
	}			
...
void Motor::ComputeForces(float delta)
{
	...
	//FPhysicsInterface::GetDriveAngularVelocity(constraintComponent_->ConstraintInstance.ConstraintHandle, currentVeloctiy);
	//currentVeloctiy *= this->radius_;
	//GEngine->AddOnScreenDebugMessage(-1, 8, FColor::Green, FString::Printf(TEXT("urdfbot speed: %s "), *currentVeloctiy.ToString()));
}
...
TMap<FString, FString> Motor::GetState()
{
	...
	////debug urdfbot speed 
	//GEngine->AddOnScreenDebugMessage(1, 1, FColor::Red, FString::Printf(TEXT("urdfbot speed: %s m/s"), *FString::SanitizeFloat(ActualSpeed)));
    return state;
}
...
{
	this->constraintComponent_->ConstraintInstance.SetAngularPositionDrive(false, isDriveEnabled);
	this->constraintComponent_->ConstraintInstance.SetAngularVelocityDrive(false, isDriveEnabled);
}
...
void Motor::ConvertTorqueToVelocity()
{
	...
	// 除以2* PI 时因为 SetAngularVelocityTarget的参数 为转的圈数
	currentSpeed /= (2 * PI);
	//GEngine->AddOnScreenDebugMessage(1, 1, FColor::Red, FString::Printf(TEXT("urdfbot speed: %s "), *FString::SanitizeFloat(ActualSpeed)));
	...
	*/
	float damping = constraintComponent_->ConstraintInstance.ProfileInstance.AngularDrive.TwistDrive.Damping;
	FVector  targetVelocity = this->targetConstTorque_ / damping + FVector(currentSpeed, 0, 0);
	this->baseLink_->GetRootMesh()->WakeAllRigidBodies();
	this->actuationLink_->GetRootMesh()->WakeAllRigidBodies();
