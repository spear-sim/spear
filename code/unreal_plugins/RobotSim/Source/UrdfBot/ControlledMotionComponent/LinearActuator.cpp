...
void LinearActuator::Attach(AUrdfLink* baseLink, AUrdfLink* actuationLink, UrdfJointSpecification *jointSpecification, UPhysicsConstraintComponent* constraintComponent)
{
	...
	float a = this->constraintComponent_->ConstraintInstance.ProfileInstance.LinearDrive.XDrive.Stiffness;
	float b = this->constraintComponent_->ConstraintInstance.ProfileInstance.LinearDrive.XDrive.Damping;
	//default stiffness and damping instead of URDF value to optimize movement performance
	URobotBlueprintLib::LogMessage(FString("prismatic")+this->name_, FString::SanitizeFloat(a)+" "+FString::SanitizeFloat(b), LogDebugLevel::Failure, 30);
	if (this->name_ == "l_gripper_finger_joint" or this->name_ == "r_gripper_finger_joint")
	{
...
void LinearActuator::ControlAbsoluteJointPose(TMap<FString, float> controlSignals)
{
	...
	if(delta < 0 || delta > 1){
		throw std::runtime_error("Servo control signal has value is outside the expected range of 0-1.");
	}
...
void LinearActuator::ControlRelativeJointPose(TMap<FString, float> controlSignals)
{
    FVector basePosition = this->baseLink_->GetActorLocation();
    FVector actuatorPosition = this->actuationLink_->GetActorLocation();
    FVector actuatorZero = basePosition + this->baseToActuatorZero_;
...
void LinearActuator::ComputeForces(float delta)
{
    ...
    if (!this->runOneTick_)
    {
		...
		UE_LOG(LogTemp, Warning, TEXT("noopControlSignal:%f"), noopControlSignal);
...
TMap<FString, FString> LinearActuator::GetState()
{
	...
	FVector actuationLinkSpeed = this->actuationLink_->GetPhysicsLinearVelocity();
...
void LinearActuator::SetDrive(float stiffness, float damping)
{
	...
	constraintComponent_->SetLinearDriveParams(stiffness, damping, stiffness * 100);
	this->constraintComponent_->ConstraintInstance.SetLinearPositionDrive(true, false, false);
	this->constraintComponent_->ConstraintInstance.SetLinearVelocityDrive(true, false, false);
}

void LinearActuator::GetDrive(float* stiffness, float* damping)
{
	*stiffness = this->constraintComponent_->ConstraintInstance.ProfileInstance.LinearDrive.XDrive.Stiffness;
	*damping = this->constraintComponent_->ConstraintInstance.ProfileInstance.LinearDrive.XDrive.Damping;
}
...
void LinearActuator::SetDriveTarget(float target)
{
	...
	float actual = 0.5 * this->range_ - (target - this->mindegree) * this->worldScale;
...
{
	UPhysicsConstraintComponent* constraintComponent = this->GetConstraintComponent();
	FVector currentTarget = constraintComponent->ConstraintInstance.ProfileInstance.LinearDrive.PositionTarget;
	return (0.5 * this->range_ - currentTarget.X) / this->worldScale + this->mindegree;
}
...
void LinearActuator::ConvertTorqueToVelocity() const
{
	...
	*/
	...
	float  targetPostition = this->targetConstTorque_.X / stiffness + CurrentLinear;
