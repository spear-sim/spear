...
public:
    ...
    virtual void Attach(AUrdfLink* baseLink,
    ...
    virtual void
    ControlAbsoluteJointPose(TMap<FString, float> controlSignals) = 0;
    virtual void
    ControlRelativeJointPose(TMap<FString, float> controlSignals) = 0;
...
protected:
    bool GetFloatFromConfiguration(FString configName,
                                   TMap<FString, FString> configuration,
                                   float& out,
                                   bool throwIfNotExist = false,
                                   bool throwIfNonNumeric = false);
