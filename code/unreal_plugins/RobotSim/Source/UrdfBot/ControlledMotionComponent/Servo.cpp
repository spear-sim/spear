...
void Servo::Attach(AUrdfLink* baseLink,
                   ...
                   UPhysicsConstraintComponent* constraintComponent)
{
    ...
    if (jointSpecification->Type != REVOLUTE_TYPE)
    {
        throw std::runtime_error(
            "Trying to initialize a servo on joint '" +
            std::string(TCHAR_TO_UTF8(*jointSpecification->Name)) +
            "', which is not of type revolute.");
    }
    ...
    if (jointSpecification->Limit->Lower > 0)
    {
        throw std::runtime_error(
            ...
            "' has a lower of '" +
            std::to_string(jointSpecification->Limit->Lower) +
            "', which is > 0.");
    }
    ...
    if (jointSpecification->Limit->Upper < 0)
    {
        throw std::runtime_error(
            "Joint '" + std::string(TCHAR_TO_UTF8(*jointSpecification->Name)) +
            "' has an upper of '" +
            std::to_string(jointSpecification->Limit->Upper) +
            "', which is < 0.");
    }

    this->rotationRange_ = FMath::RadiansToDegrees(
...
void Servo::SetControl(TMap<FString, float> controlSignals)
{
    ...
    if (value < 0 || value > 1)
        throw std::runtime_error(
            "Servo control signal has value '" + std::to_string(value) +
...
void Servo::ControlRelativeJointPose(TMap<FString, float> controlSignals)
{
    ...
    error =
        std::max(std::min(error, maxRotationAmount), -1.0f * maxRotationAmount);
    this->actualServoPosition_ += error;

    // lower < 0 ,  upper > 0 ;  必须要讲actualPosition 从 0~1  映射成 -0.5 ~
    // 0.5
    float rotateDegrees = (this->actualServoPosition_ * this->rotationRange_) -
                          (0.5 * this->rotationRange_);
    FRotator setRotator = FRotator(0, 0, rotateDegrees);
    this->actuationLink_->GetRootMesh()->WakeAllRigidBodies();
    this->baseLink_->GetRootMesh()->WakeAllRigidBodies();
    this->constraintComponent_->SetAngularOrientationTarget(setRotator);
...
void Servo::ComputeForces(float delta)
{
    ...
    if (!this->runOneTick_)
    {
        ...
        float noopControlSignal =
            (0.0f - this->lower_) * this->invRotationRange_;
        ...
        else if (noopControlSignal > 1.0f)
        {
            throw std::runtime_error(
                "The initial state of the actuator could not be resolved. The "
                "actuator is extended longer than the maximal allowable "
                "distance.");
        }
    ...
    {
        float error =
            (this->controlSignalSetPoint_ - this->actualServoPosition_);
        float maxRotationAmount = this->maxRotationRateSpeed_ * delta;

        //对revolue joint的运动速度进行限制。
        error = std::max(std::min(error, maxRotationAmount),
                         -1.0f * maxRotationAmount);
        this->actualServoPosition_ += error;

        // lower < 0 ,  upper > 0 ;  必须要讲actualPosition 从 0~1  映射成 -0.5
        // ~ 0.5
        float rotateDegrees =
            (this->actualServoPosition_ * this->rotationRange_) -
            (0.5 * this->rotationRange_);
        FRotator setRotator = FRotator(0, 0, rotateDegrees);
        this->actuationLink_->GetRootMesh()->WakeAllRigidBodies();
        this->baseLink_->GetRootMesh()->WakeAllRigidBodies();
...
void Servo::SetDriveTarget(float target)
{
    ...
    else
    {
        ...
        float actual =
            (target * 180 / PI - this->lower_) - this->rotationRange_ * 0.5;
...
float Servo::GetDriveTarget()
{
    if (this->mindegree + this->maxdegree == 0)
    {
        FRotator currentTarget = this->GetConstraintComponent()
                                     ->ConstraintInstance.ProfileInstance
                                     .AngularDrive.OrientationTarget;
...
void Servo::ConvertTorqueToVelocity() const
{
    ...
    */
    ...
    float targetOritation =
        this->targetConstTorque_.X / stiffness + CurrentDegrees;
