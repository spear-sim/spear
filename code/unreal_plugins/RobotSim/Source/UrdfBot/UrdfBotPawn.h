...
class ROBOTSIM_API AUrdfBotPawn : public APawn, public RobotSimVehicle
{
	...
	public:
		...
		UFUNCTION()
		void OnEndEffectorCollision(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit);
		TMap<FString, TTuple<UrdfJointType, UPhysicsConstraintComponent*>>		getConstraints();

	public:
		void setLinkForceAndTorque(FString componentName, const FVector& force, const FVector& torque);
		...
		// For detail explanation, see https://docs.nvidia.com/gameworks/content/gameworkslibrary/physx/guide/Manual/Joints.html#drives
		void onStiffnessUp();
		...
		FTransform getRelativePose(const FString& baseLinkName, const FString& TargetlinkName);
    ...
    private:
        ...
        TMap<FString, TTuple<UrdfJointType, UPhysicsConstraintComponent*>> constraints_;  //所有joint节点， 包括可控制的和fixed的类型。
		TMap<FString, ControlledMotionComponent*> controlled_motion_components_; // 所有关于可控制joint节点
		TMap<FString, ControlledMotionComponent*> movement_components_; // 所有关于移动的joint节点
        StaticMeshGenerator staticMeshGenerator_;
		InverseKinematicComponent* inverseKinematicComponent = new InverseKinematicComponent();
