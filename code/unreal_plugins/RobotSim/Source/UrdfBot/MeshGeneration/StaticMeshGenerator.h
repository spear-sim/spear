...
{
public:
    bool Initialize(UStaticMesh* boxTemplateMesh,
                    UStaticMesh* cylinderTemplateMesh,
                    UStaticMesh* sphereTemplateMesh,
                    TMap<FString, UStaticMesh*> unrealMeshes);
    bool
    CreateUnscaledMeshForLink(const UrdfLinkSpecification& linkSpecification,
...
private:
    void ParseProceduralMeshSpecification(
        FString fileName,
    ...
    // The below functions are mostly copy-pasted from
    // Editor\UnrealEd\Public\BSPOps.cpp, with minor edits. We cannot take a
    // dependency on UnrealEd, as it will not allow us to package our final
    // build. So, we duplicate the necessary functionality into this library.

    enum ENodePlace
    {
        NODE_Back =
            0, // Node is in back of parent              -> Bsp[iParent].iBack.
        NODE_Front =
            1, // Node is in front of parent             -> Bsp[iParent].iFront.
        NODE_Plane =
            2, // Node is coplanar with parent           -> Bsp[iParent].iPlane.
    ...
    struct FBspPointsKey
    {
        ...
        FBspPointsKey(int32 InX, int32 InY, int32 InZ): X(InX), Y(InY), Z(InZ)
        {
        }

        friend FORCEINLINE bool operator==(const FBspPointsKey& A,
                                           const FBspPointsKey& B)
        {
        ...
        {
            return HashCombine(static_cast<uint32>(Key.X),
                               HashCombine(static_cast<uint32>(Key.Y),
                                           static_cast<uint32>(Key.Z)));
        }
    };
    ...
    {
    public:
        FBspPointsGrid(float InGranularity,
                       float InThreshold,
                       int32 InitialSize = 0)
        ...
        int32
        FindOrAddPoint(const FVector& Point, int32 Index, float Threshold);
    ...
    private:
        ...
        int32 GetAdjacentIndexIfOverlapping(int32 GridIndex,
                                            float GridPos,
                                            float GridThreshold);
    };
    ...
    void
    GenerateBspCollisionMesh(ProceduralMeshSpecification& meshSpecification,
                             UModel* model);
    TArray<TArray<FVector>>
    CreateCollisionVAHCD(TArray<FVector> stlPoints,
    ...
    void SaveVhacdGeneratedCollision(TArray<TArray<FVector>> meshes,
                                     FString outputFolder);
    TArray<TArray<FPoly>> StaticMeshGeneratorBspBuild(UModel* TempModel,
                                                      ...
                                                      int32 iNode);
    TArray<TArray<FPoly>>
    StaticMeshGeneratorSplitPolyList(UModel* TempModel,
                                     TArray<FPoly> ModelPolys,
                                     int32 iParent,
    ...
    void TagReferencedNodes(UModel* Model,
                            int32* NodeRef,
                            int32* PolyRef,
                            int32 iNode);
    void SplitPartitioner(UModel* Model,
                          FPoly** PolyList,
                          FPoly** FrontList,
                          FPoly** BackList,
                          int32 n,
    ...
    void UpdateConvolutionWithPolys(UModel* Model,
                                    int32 iNode,
                                    FPoly** PolyList,
                                    int32 nPolys);
    void SplitPolyList(UModel* Model,
                       int32 iParent,
                       ENodePlace NodePlace,
                       int32 NumPolys,
                       FPoly** PolyList,
                       ...
                       int32 RebuildSimplePolys,
                       FBspPointsGridContainer* GridContainer);
    FPoly* FindBestSplit(int32 NumPolys,
                         FPoly** PolyList,
                         EBspOptimization Opt,
                         int32 Balance,
                         int32 InPortalBias);
    int32 BspAddNode(UModel* Model,
                     int32 iParent,
                     ENodePlace NodePlace,
                     uint32 NodeFlags,
                     FPoly* EdPoly,
                     FBspPointsGridContainer* GridContainer);
    int32 BspAddPoint(UModel* Model,
                      FVector* V,
                      FBspPointsGridContainer* GridContainer,
                      bool Exact);
    int32 BspAddVector(UModel* Model,
                       FVector* V,
                       FBspPointsGridContainer* GridContainer,
                       bool Exact);
    int32
    AddThing(TArray<FVector>& Vectors, FVector& V, float Thresh, int32 Check);
};
