...
bool StaticMeshGenerator::Initialize(UStaticMesh* boxTemplateMesh,
                                     UStaticMesh* cylinderTemplateMesh,
                                     UStaticMesh* sphereTemplateMesh,
                                     TMap<FString, UStaticMesh*> unrealMeshes)
{
...
bool StaticMeshGenerator::CreateUnscaledMeshForLink(
    const UrdfLinkSpecification& linkSpecification,
    UrdfGeometry* visualGeometry,
    UrdfGeometry* collisionGeometry,
    APawn* outer,
    AUrdfLink* link,
    TMap<FString, UMaterialInterface*> materials)
{
    ...
    FString linkMaterialName =
        linkSpecification.VisualSpecification->MaterialName;
    FName collisionName =
        FName((linkName + TEXT("_collision")).GetCharArray().GetData());
    ...
    if (visualGeometry->GetGeometryType() == MESH &&
        collisionGeometry->GetGeometryType() == MESH)
    {
        UrdfMesh* visual = static_cast<UrdfMesh*>(visualGeometry);
        UrdfMesh* collision = static_cast<UrdfMesh*>(collisionGeometry);
        reparseMeshGeometry =
            ((!visual->FileLocation.Equals(collision->FileLocation)) ||
             (visual->FileType != collision->FileType));
    }

    UrdfGeometryType geometryType = visualGeometry->GetGeometryType();
    FName meshName((linkName + TEXT("_visual")).GetCharArray().GetData());
    if (geometryType == BOX || geometryType == CYLINDER ||
        geometryType == SPHERE)
    {
        UStaticMeshComponent* meshComponent =
            NewObject<UStaticMeshComponent>(outer, meshName);
        switch (geometryType)
        {
        case BOX:
    ...
    else if (geometryType == MESH)
    {
        ...
        if (meshSpecification->FileType == UNREAL_MESH)
        {
            ...
            if (!this->unrealMeshes_.Contains(meshSpecification->FileLocation))
            {
                throw std::runtime_error(
                    "Unable to find static mesh '" +
                    std::string(
                        TCHAR_TO_UTF8(*meshSpecification->FileLocation)) +
                    "' in StaticMeshGenerator.");
            }

            UStaticMesh* staticMesh =
                this->unrealMeshes_[meshSpecification->FileLocation];
        ...
        {
            this->ParseProceduralMeshSpecification(
                meshSpecification->FileLocation, meshSpecification->FileType,
                meshSpecification->ReverseNormals,
                meshSpecification->ScaleFactor, proceduralMeshSpecification);

            UProceduralMeshComponent* meshComponent =
                NewObject<UProceduralMeshComponent>(outer, meshName);
            meshComponent->CreateMeshSection_LinearColor(
                0, proceduralMeshSpecification.Verticies,
                proceduralMeshSpecification.Triangles, TArray<FVector>(),
                TArray<FVector2D>(), TArray<FLinearColor>(),
            ...
            UrdfLinkCollisionSpecification* urdfCollisionSpecification =
            ...
            if (meshSpecification->DynamicCollisionType == COL_BSP)
            {
                ...
                {
                    this->ParseProceduralMeshSpecification(
                        meshSpecification->FileLocation,
                        meshSpecification->FileType,
                        meshSpecification->ReverseNormals,
                        meshSpecification->ScaleFactor,
                ...
                this->GenerateBspCollisionMesh(proceduralMeshSpecification,
                                               tempModel);
                ...
                {
                    meshComponent->AddCollisionConvexMesh(
                        bs->AggGeom.ConvexElems[i].VertexData);
                }
            }
            else if (meshSpecification->DynamicCollisionType == COL_VHACD)
            {
                ...
                {
                    meshSpecification =
                        static_cast<UrdfMesh*>(collisionGeometry);
                    this->ParseProceduralMeshSpecification(
                        meshSpecification->FileLocation,
                        meshSpecification->FileType,
                        meshSpecification->ReverseNormals,
                ...
                for (int i = 0; i < proceduralMeshSpecification.Triangles.Num();
                     i++)
                {
                    triangles.Add(proceduralMeshSpecification.Triangles[i]);
                }

                TArray<TArray<FVector>> vhacdCollision =
                    this->CreateCollisionVAHCD(
                        proceduralMeshSpecification.Verticies, triangles,
                        meshSpecification->VhacdConcavity,
                        meshSpecification->VhacdResolution,
                        meshSpecification->VhacdMaxNumVerticesPerCh,
                        meshSpecification->VhacdMinVolumePerCh);

                meshComponent->SetCollisionConvexMeshes(vhacdCollision);
            }
            else if (meshSpecification->DynamicCollisionType == COL_MANUAL)
            {
                TArray<TArray<FVector>> manualCollision =
                    this->ReadManualCollision(meshSpecification->FileLocation);
                meshComponent->SetCollisionConvexMeshes(manualCollision);
            }
            // TODO   添加基本碰撞体支持
            else if (collisionGeometry->GetGeometryType() ==
                     UrdfGeometryType::BOX)
            {
                ...
                FRotator visualOffsetRotator = FMath::RadiansToDegrees(
                    urdfCollisionSpecification->Origin.RollPitchYaw);
                FPositionVertexBuffer* VertexBuffer =
                    &(boxTemplateMesh_->RenderData->LODResources[0]
                          .VertexBuffers.PositionVertexBuffer);
                if (VertexBuffer)
                {
                    for (uint32 index = 0;
                         index < VertexBuffer->GetNumVertices(); index++)
                    {
                        FVector originPoint =
                            VertexBuffer->VertexPosition(index);
                        FVector scalePoint(originPoint.X * boxGeometry->Size.X,
                                           originPoint.Y * boxGeometry->Size.Y,
                                           originPoint.Z * boxGeometry->Size.Z);
                        FVector rotatedVector =
                            visualOffsetRotator.RotateVector(scalePoint);
                ...
                meshComponent->AddCollisionConvexMesh(collisionShapeVertices);
                     ...
                     UrdfGeometryType::CYLINDER)
            {
                collisionShapeVertices.Empty();
                UrdfCylinder* cylinderGeometry =
                    static_cast<UrdfCylinder*>(collisionGeometry);
                FRotator visualOffsetRotator = FMath::RadiansToDegrees(
                    urdfCollisionSpecification->Origin.RollPitchYaw);
                FPositionVertexBuffer* VertexBuffer =
                    &(cylinderTemplateMesh_->RenderData->LODResources[0]
                          .VertexBuffers.PositionVertexBuffer);
                if (VertexBuffer)
                {
                    for (uint32 index = 0;
                         index < VertexBuffer->GetNumVertices(); index++)
                    {
                        FVector originPoint =
                            VertexBuffer->VertexPosition(index);
                        FVector scalePoint(
                            originPoint.X * cylinderGeometry->Radius * 2.0,
                            originPoint.Y * cylinderGeometry->Radius * 2.0,
                            originPoint.Z * cylinderGeometry->Length);
                        FVector rotatedVector =
                            visualOffsetRotator.RotateVector(scalePoint);
                        collisionShapeVertices.Add(rotatedVector);
                    }
            ...
            {
                throw std::runtime_error(
                    "Unrecognized collision type in StaticMeshGenerator.");
            }
        ...
        {
            throw std::runtime_error(
                "Unrecognized Geomtry FileType in StaticMeshGenerator.");
        }
    }
...
StaticMeshGenerator::CreateCollisionVAHCD(TArray<FVector> stlPoints,
                                          TArray<uint32> stlIndices,
                                          double concavity,
                                          unsigned int resolution,
                                          unsigned int maxNumVerticiesPerCH,
                                          double minVolumePerCh)
{
    VHACD::IVHACD* vhacdInterface = VHACD::CreateVHACD();

    VHACD::IVHACD::Parameters VHACD_Params;
    VHACD_Params.m_concavity =
        concavity; // Maximum allowed concavity (default=0.0025, range=0.0-1.0)
    VHACD_Params.m_resolution =
        resolution; // Maximum number of voxels generated during the
                    // voxelization stage (default=100,000,
                    // range=10,000-16,000,000)
    VHACD_Params.m_maxNumVerticesPerCH =
        maxNumVerticiesPerCH; // Controls the maximum number of triangles per
                              // convex-hull (default=64, range=4-1024)
    VHACD_Params.m_oclAcceleration = false;
    VHACD_Params.m_minVolumePerCH =
        minVolumePerCh; // this should be around 1 / (3 * m_resolution ^ (1/3))
    ...
    bool success =
        vhacdInterface->Compute(verts, numVerts, tris, numTris, VHACD_Params);
    // bool success = vhacdInterface->Compute(verts, 3, numVerts, tris, 3,
    // numTris, VHACD_Params);
...
TArray<TArray<FVector>>
StaticMeshGenerator::ReadManualCollision(FString fileListPath)
{
    TArray<TArray<FVector>> output;
    ...
    try
    {
        ...
        while (
            ProceduralMeshFileUtilities::ReadNextLineTrimmingLeadingWhitespace(
                rootFile, collisionFilePath, 256) ||
            collisionFilePath.Len() > 0)
        {
            ...
            try
            {
                collisionFile = ProceduralMeshFileUtilities::OpenFile(
                    collisionFilePath, true);

                FString line;
                while (ProceduralMeshFileUtilities::
                           ReadNextLineTrimmingLeadingWhitespace(collisionFile,
                                                                 line, 128) ||
                       line.Len() > 0)
                {
                    ...
                    {
                        collisionPoint =
                            FVector(FCString::Atof(*splitString[0]),
                    ...
                    {
                        throw std::runtime_error(
                            "Error parsing 3-d vector from string " +
...
void StaticMeshGenerator::SaveVhacdGeneratedCollision(
    TArray<TArray<FVector>> meshes, FString outputFolder)
{
    IFileHandle* headerFile = nullptr;
    outputFolder = outputFolder.Replace(TEXT("\\"), TEXT("/"));
    FString headerOutputPath =
        ProceduralMeshFileUtilities::DeepCopyFString(outputFolder);
    ...
    {
        headerFile =
            ProceduralMeshFileUtilities::OpenFile(headerOutputPath, false);

        for (int meshNum = 0; meshNum < meshes.Num(); meshNum++)
        {
            IFileHandle* meshFile = nullptr;
            FString meshFileName =
                TEXT("collision_") + FString::FromInt(meshNum) + ".txt";
            ...
            try
            {
                meshFile = ProceduralMeshFileUtilities::OpenFile(meshOutputPath,
                                                                 false);
                ...
                for (int i = 0; i < currentMesh.Num(); i++)
                {
                    ...
                    meshFile->Write((const uint8*)TCHAR_TO_UTF8(*outputString),
                                    outputString.Len());
                }
                ...
                headerFile->Write((const uint8*)TCHAR_TO_UTF8(*fileNameLine),
                                  fileNameLine.Len());
            }
            catch (std::exception& e)
            {
...
void StaticMeshGenerator::ParseProceduralMeshSpecification(
    FString fileName,
    ...
    ProceduralMeshSpecification& meshSpecification)
{
    ...
    TSharedPtr<ProceduralMeshFileParser> meshFileParser =
        parserFactory.Create(fileType);
    ...
    // Some packages (e.g. Blender) spits out the verts in the opposite order
    // that Unreal expects. This means that the normal is facing the wrong way,
    // and we will be looking at the back side of each of the faces.
    if (reverseNormals)
    {
        for (int i = 0; i < meshSpecification.Triangles.Num(); i = i + 3)
        {
            std::swap(meshSpecification.Triangles[i],
                      meshSpecification.Triangles[i + 2]);
        }
    }
}

void StaticMeshGenerator::SetupPhysicsForDefaultComponent(
...
void StaticMeshGenerator::GenerateBspCollisionMesh(
    ProceduralMeshSpecification& meshSpecification, UModel* model)
{
    ...
    for (int i = 0; i < meshSpecification.Triangles.Num(); i = i + 3)
    {
        ...
        //
        // To detect collinearity, we use the fact that give three collinear
        // points, A, B, and C, (A->B) X (A->C) = 0 if the points are collinear.
        //
        // Also: the offsets of these three vectors into the Triangles array is
        // important - don't change them! Otherwise, the collision mesh won't
        // render.
        FVector first =
            meshSpecification.Verticies[meshSpecification.Triangles[i + 2]];
        FVector second =
            meshSpecification.Verticies[meshSpecification.Triangles[i + 1]];
        FVector third =
            meshSpecification.Verticies[meshSpecification.Triangles[i + 0]];
        ...
        if (checkVec.SizeSquared() >= (float)THRESH_ZERO_NORM_SQUARED)
        {
            ...
            if (success != 0)
            {
                std::string message = "Error: corrupt st. file. Computation of "
                                      "normals failed. Vertices are:\n";
                message += "<" + std::to_string(first.X) + ", " +
                           std::to_string(first.Y) + ", " +
                           std::to_string(first.Z) + ">\n";
                message += "<" + std::to_string(second.X) + ", " +
                           std::to_string(second.Y) + ", " +
                           std::to_string(second.Z) + ">\n";
                message += "<" + std::to_string(third.X) + ", " +
                           std::to_string(third.Y) + ", " +
                           std::to_string(third.Z) + ">\n";
...
TArray<TArray<FPoly>>
StaticMeshGenerator::StaticMeshGeneratorBspBuild(UModel* TempModel,
                                                 TArray<FPoly> ModelPolys,
                                                 ...
                                                 int32 iNode)
{
    ...
    // Answer: No. No, this shouldn't. For large meshes, this can cause a stack
    // overflow.
    FBspPointsGrid pointsGrid(50.0f, THRESH_POINTS_ARE_SAME);
    FBspPointsGrid vectorsGrid(
        1.0f / 16.0f,
        FMath::Max(THRESH_NORMALS_ARE_SAME, THRESH_VECTORS_ARE_NEAR));
    ...
    if (OriginalPolys)
    {
        ...
        splitPolys = StaticMeshGeneratorSplitPolyList(
            TempModel, ModelPolys, INDEX_NONE, NODE_Root, ModelPolys.Num(),
            PolyList, Opt, Balance, PortalBias, RebuildSimplePolys,
            &gridContainer);
...
TArray<TArray<FPoly>> StaticMeshGenerator::StaticMeshGeneratorSplitPolyList(
    UModel* TempModel,
    TArray<FPoly> ModelPolys,
    int32 iParent,
    ...
    FBspPointsGridContainer* GridContainer)
{
    ...
    FPoly* SplitPoly =
        FindBestSplit(NumPolys, PolyList, Opt, Balance, PortalBias);

    // Add the splitter poly to the Bsp with either a new BspSurf or an existing
    // one.
    ...
    int32 iOurNode =
        BspAddNode(TempModel, iParent, NodePlace, 0, SplitPoly, GridContainer);
    ...
    for (int32 i = 0; i < NumPolys; i++)
    {
        ...
        switch (EdPoly->SplitWithPlane(SplitPoly->Vertices[0],
                                       SplitPoly->Normal, FrontEdPoly,
                                       BackEdPoly, 1))
        {
        case SP_Coplanar:
            if (RebuildSimplePolys)
            ...
            iPlaneNode = BspAddNode(TempModel, iPlaneNode, NODE_Plane, 0,
                                    EdPoly, GridContainer);
            break;
    ...
    if (NumFront > 0)
        StaticMeshGeneratorSplitPolyList(
            TempModel, ModelPolys, iOurNode, NODE_Front, NumFront, FrontList,
            Opt, Balance, PortalBias, RebuildSimplePolys, GridContainer);
    if (NumBack > 0)
        StaticMeshGeneratorSplitPolyList(
            TempModel, ModelPolys, iOurNode, NODE_Back, NumBack, BackList, Opt,
            Balance, PortalBias, RebuildSimplePolys, GridContainer);

    // Delete FPolys allocated above. We cannot use FMemStack::Get() for FPoly
    // as the array data FPoly contains will be allocated in regular memory.
...
void StaticMeshGenerator::BspBuildBounds(UModel* Model)
{
    ...
    new (Polys[0].Vertices)
        FVector(-HALF_WORLD_MAX, -HALF_WORLD_MAX, HALF_WORLD_MAX);
    new (Polys[0].Vertices)
        FVector(HALF_WORLD_MAX, -HALF_WORLD_MAX, HALF_WORLD_MAX);
    new (Polys[0].Vertices)
        FVector(HALF_WORLD_MAX, HALF_WORLD_MAX, HALF_WORLD_MAX);
...
void StaticMeshGenerator::TagReferencedNodes(UModel* Model,
                                             int32* NodeRef,
                                             int32* PolyRef,
                                             int32 iNode)
{
...
void StaticMeshGenerator::SplitPartitioner(UModel* Model,
                                           FPoly** PolyList,
                                           FPoly** FrontList,
                                           FPoly** BackList,
                                           int32 n,
                                           ...
                                           TArray<FPoly*>& AllocatedFPolys)
{
    ...
    while (n < nPolys)
    {
        ...
        switch (InfiniteEdPoly.SplitWithPlane(Poly->Vertices[0], Poly->Normal,
                                              &FrontPoly, &BackPoly, 0))
        {
        case SP_Coplanar:
            // May occasionally happen.
            //				UE_LOG(LogBSPOps, Log,  TEXT("FilterBound: Got
            //inficoplanar") );
            break;
...
void StaticMeshGenerator::FilterBound(UModel* Model,
                                      FBox* ParentBound,
                                      int32 iNode,
                                      FPoly** PolyList,
                                      int32 nPolys,
                                      int32 Outside)
{
    ...
    for (int32 i = 0; i < nPolys; i++)
    {
        ...
        default:
            // UE_LOG(LogBSPOps, Fatal, TEXT("FZoneFilter::FilterToLeaf: Unknown
            // split code"));
            break;
        }
    }
    if (nFront && nBack)
    {
        ...
        SplitPartitioner(Model, PolyList, FrontList, BackList, 0, nPolys,
                         nFront, nBack, InfiniteEdPoly, AllocatedFPolys);
    }
    else
    {
        // 		if( !nFront ) UE_LOG(LogBSPOps, Log,  TEXT("FilterBound:
        // Empty fronthull") ); 		if( !nBack  ) UE_LOG(LogBSPOps, Log,
        // TEXT("FilterBound: Empty backhull") );
    }
    ...
    {
        if (Node.iFront != INDEX_NONE)
            FilterBound(Model, &Bound, Node.iFront, FrontList, nFront,
                        Outside || Node.IsCsg());
    ...
    {
        if (Node.iBack != INDEX_NONE)
            FilterBound(Model, &Bound, Node.iBack, BackList, nBack,
                        Outside && !Node.IsCsg());
        else if (Outside && !Node.IsCsg())
...
void StaticMeshGenerator::UpdateBoundWithPolys(FBox& Bound,
                                               FPoly** PolyList,
                                               int32 nPolys)
{
    for (int32 i = 0; i < nPolys; i++)
...
void StaticMeshGenerator::UpdateConvolutionWithPolys(UModel* Model,
                                                     int32 iNode,
                                                     FPoly** PolyList,
                                                     int32 nPolys)
{
...
void StaticMeshGenerator::SplitPolyList(UModel* Model,
                                        int32 iParent,
                                        ENodePlace NodePlace,
                                        int32 NumPolys,
                                        FPoly** PolyList,
                                        ...
                                        FBspPointsGridContainer* GridContainer)
{
    ...
    int32 iOurNode =
        BspAddNode(Model, iParent, NodePlace, 0, SplitPoly, GridContainer);
    int32 iPlaneNode = iOurNode;
    ...
    // Recursively split the front and back pools.
    if (NumFront > 0)
        SplitPolyList(Model, iOurNode, NODE_Front, NumFront, FrontList, Opt,
                      Balance, PortalBias, RebuildSimplePolys, GridContainer);
    if (NumBack > 0)
        SplitPolyList(Model, iOurNode, NODE_Back, NumBack, BackList, Opt,
                      Balance, PortalBias, RebuildSimplePolys, GridContainer);
...
FPoly* StaticMeshGenerator::FindBestSplit(int32 NumPolys,
                                          FPoly** PolyList,
                                          EBspOptimization Opt,
                                          int32 Balance,
                                          int32 InPortalBias)
{
    ...
    Balance &= 0xFF; // keep only the low byte to recover "Balance"
                     // UE_LOG(LogBSPOps, Log, TEXT("Balance=%d PortalBias=%f"),
                     // Balance, PortalBias );

    if (Opt == BSP_Optimal)
        Inc = 1; // Test lots of nodes.
    else if (Opt == BSP_Good)
    ...
    for (i = 0; i < NumPolys; i += Inc)
    {
        ...
        do
        {
            ...
            Poly = PolyList[Index];
                 ...
                 ((Poly->PolyFlags & PF_AddLast) &&
                  !(Poly->PolyFlags & PF_Portal)) &&
                 !AllSemiSolids);
        ...
        for (j = 0; j < NumPolys; j += Inc)
            if (j != Index)
            {
                ...
                switch (OtherPoly->SplitWithPlaneFast(
                    FPlane(Poly->Vertices[0], Poly->Normal), NULL, NULL))
                {
                case SP_Coplanar:
                    Coplanar++;
                    break;
        ...
        // Score optimization: minimize cuts vs. balance tree (as specified in
        // BSP Rebuilder dialog)
        Score = (100.0 - float(Balance)) * Splits +
                float(Balance) * FMath::Abs(Front - Back);
        if (Poly->PolyFlags & PF_Portal)
        {
            // PortalBias -- added by Legend on 4/12/2000
            //
            // PortalBias enables level designers to control the effect of
            // Portals on the BSP. This effect can range from 0.0 (ignore
            // portals), to 1.0 (portals cut everything).
            //
            // In builds prior to this (since the 221 build dating back to
            // 1/31/1999) the bias has been 1.0 causing the portals to cut the
            // BSP in ways that will potentially degrade level performance, and
            // increase the BSP complexity.
            //
            // By setting the bias to a value between 0.3 and 0.7 the positive
            // effects of the portals are preserved without giving them
            // unreasonable priority in the BSP.
            //
            // Portals should be weighted high enough in the BSP to separate
            // major parts of the level from each other (pushing entire rooms
            // down the branches of the BSP), but should not be so high that
            // portals cut through adjacent geometry in a way that increases
            // complexity of the room being (typically, accidentally) cut.
            //
            Score -= (100.0 - float(Balance)) * Splits *
                     PortalBias; // ignore PortalBias of the split polys -- bias
                                 // toward portal selection for cutting planes!
        }
        // UE_LOG(LogBSPOps, Log,  "  %4d: Score = %f (Front = %4d, Back = %4d,
        // Splits = %4d, Flags = %08X)", Index, Score, Front, Back, Splits,
        // Poly->PolyFlags ); //LEC
...
int32 StaticMeshGenerator::BspAddNode(UModel* Model,
                                      int32 iParent,
                                      ENodePlace NodePlace,
                                      uint32 NodeFlags,
                                      FPoly* EdPoly,
                                      FBspPointsGridContainer* GridContainer)
{
    ...
    if (EdPoly->iLinkSurf == Model->Surfs.Num())
    {
        ...
        Surf->vTextureU =
            BspAddVector(Model, &EdPoly->TextureU, GridContainer, 0);
        Surf->vTextureV =
            BspAddVector(Model, &EdPoly->TextureV, GridContainer, 0);
        ...
        if (Model->LightmassSettings.Find(EdPoly->LightmassSettings,
                                          FoundLightmassIndex) == false)
        {
            FoundLightmassIndex =
                Model->LightmassSettings.Add(EdPoly->LightmassSettings);
        }
    ...
    {
        // Split up into two coplanar sub-polygons (one with MAX_NODE_VERTICES
        // vertices and one with all the remaining vertices) and recursively add
        // them.
        ...
        *EdPoly1 = *EdPoly;
        EdPoly1->Vertices.RemoveAt(FBspNode::MAX_NODE_VERTICES,
                                   EdPoly->Vertices.Num() -
                                       FBspNode::MAX_NODE_VERTICES);

        // EdPoly2 is the first vertex from EdPoly, and the last
        // EdPoly->Vertices.Num() - MAX_NODE_VERTICES + 1.
        FPoly* EdPoly2 = new FPoly;
        *EdPoly2 = *EdPoly;
        EdPoly2->Vertices.RemoveAt(1, FBspNode::MAX_NODE_VERTICES - 2);

        int32 iNode = BspAddNode(Model, iParent, NodePlace, NodeFlags, EdPoly1,
                                 GridContainer); // Add this poly first.
        BspAddNode(Model, iNode, NODE_Plane, NodeFlags, EdPoly2,
                   GridContainer); // Then add other (may be bigger).
    ...
    else
    {
        ...
        // Add all points to point table, merging nearly-overlapping polygon
        // points with other points in the poly to prevent criscrossing vertices
        // from being generated.

        // Must maintain Node->NumVertices on the fly so that bspAddPoint is
        // always called with the Bsp in a clean state.
        Node.NumVertices = 0;
        FVert* VertPool = &Model->Verts[Node.iVertPool];
        for (uint8 i = 0; i < EdPoly->Vertices.Num(); i++)
        {
            int32 pVertex =
                BspAddPoint(Model, &EdPoly->Vertices[i], GridContainer, 0);
            if (Node.NumVertices == 0 ||
                VertPool[Node.NumVertices - 1].pVertex != pVertex)
            {
        ...
        if (Node.NumVertices >= 2 &&
            VertPool[0].pVertex == VertPool[Node.NumVertices - 1].pVertex)
        {
            Node.NumVertices--;
        }
...
int32 StaticMeshGenerator::BspAddPoint(UModel* Model,
                                       FVector* V,
                                       FBspPointsGridContainer* GridContainer,
                                       bool Exact)
{
    const float Thresh =
        Exact ? THRESH_POINTS_ARE_SAME : THRESH_POINTS_ARE_NEAR;

    if (GridContainer->PointsGrid)
    {
        // If a points grid has been built for quick point lookup, use that
        // instead of doing a linear search
        const int32 NextIndex = Model->Points.Num();
        // Always look for points with a low threshold; a generous threshold can
        // result in 'leaks' in the BSP and unwanted polys being generated
        const int32 ReturnedIndex = GridContainer->PointsGrid->FindOrAddPoint(
            *V, NextIndex, THRESH_POINTS_ARE_SAME);
    ...
    // Try to find a match quickly from the Bsp. This finds all potential
    // matches except for any dissociated from nodes/surfaces during a rebuild.
    FVector Temp;
    int32 pVertex;
...
int32 StaticMeshGenerator::BspAddVector(UModel* Model,
                                        FVector* V,
                                        FBspPointsGridContainer* GridContainer,
                                        bool Exact)
{
    ...
    {
        // If a points grid has been built for quick vector lookup, use that
        // instead of doing a linear search
        const int32 NextIndex = Model->Vectors.Num();
        const int32 ReturnedIndex =
            GridContainer->VectorsGrid->FindOrAddPoint(*V, NextIndex, Thresh);
        if (ReturnedIndex == NextIndex)
        {
            Model->Vectors.Add(*V);
        }
    ...
    return AddThing(Model->Vectors, *V,
                    Exact ? THRESH_NORMALS_ARE_SAME : THRESH_VECTORS_ARE_NEAR,
                    1);
}

int32 StaticMeshGenerator::AddThing(TArray<FVector>& Vectors,
                                    FVector& V,
                                    float Thresh,
                                    int32 Check)
{
...
int32 StaticMeshGenerator::FBspPointsGrid::FindOrAddPoint(const FVector& Point,
                                                          int32 Index,
                                                          float PointThreshold)
{
    // Offset applied to the grid coordinates so aligned vertices (the normal
    // case) don't overlap several grid items (taking into account the
    // threshold)
    ...
    FBspPointsGridItem& GridItem =
        GridMap.FindOrAdd(FBspPointsKey(GridIndexX, GridIndexY, GridIndexZ));

    // Iterate through grid item points and return a point if it's close to the
    // threshold
    ...
    {
        if (FVector::DistSquared(IndexedPoint.Point, Point) <=
            PointThresholdSquared)
        {
    ...
    // The grid has a maximum threshold of a certain radius. If the point is
    // near the edge of a grid cube, it may overlap into other items. Add it to
    // all grid items it can be seen from.
    const float GridThreshold = Threshold * OneOverGranularity;
    const int32 NeighbourX =
        GetAdjacentIndexIfOverlapping(GridIndexX, GridX, GridThreshold);
    const int32 NeighbourY =
        GetAdjacentIndexIfOverlapping(GridIndexY, GridY, GridThreshold);
    const int32 NeighbourZ =
        GetAdjacentIndexIfOverlapping(GridIndexZ, GridZ, GridThreshold);
    ...
    if (bOverlapsInX)
    {
        GridMap.FindOrAdd(FBspPointsKey(NeighbourX, GridIndexY, GridIndexZ))
            .IndexedPoints.Emplace(Point, Index);

        if (bOverlapsInY)
        {
            GridMap.FindOrAdd(FBspPointsKey(NeighbourX, NeighbourY, GridIndexZ))
                .IndexedPoints.Emplace(Point, Index);
            ...
            if (bOverlapsInZ)
            {
                GridMap
                    .FindOrAdd(
                        FBspPointsKey(NeighbourX, NeighbourY, NeighbourZ))
                    .IndexedPoints.Emplace(Point, Index);
            }
        }
        else if (bOverlapsInZ)
        {
            GridMap.FindOrAdd(FBspPointsKey(NeighbourX, GridIndexY, NeighbourZ))
                .IndexedPoints.Emplace(Point, Index);
        }
    }
    else
    {
        if (bOverlapsInY)
        {
            GridMap.FindOrAdd(FBspPointsKey(GridIndexX, NeighbourY, GridIndexZ))
                .IndexedPoints.Emplace(Point, Index);
            ...
            if (bOverlapsInZ)
            {
                GridMap
                    .FindOrAdd(
                        FBspPointsKey(GridIndexX, NeighbourY, NeighbourZ))
                    .IndexedPoints.Emplace(Point, Index);
            }
        }
        else if (bOverlapsInZ)
        {
            GridMap.FindOrAdd(FBspPointsKey(GridIndexX, GridIndexY, NeighbourZ))
                .IndexedPoints.Emplace(Point, Index);
        }
    }
...
FORCEINLINE int32
StaticMeshGenerator::FBspPointsGrid::GetAdjacentIndexIfOverlapping(
    int32 GridIndex, float GridPos, float GridThreshold)
{
