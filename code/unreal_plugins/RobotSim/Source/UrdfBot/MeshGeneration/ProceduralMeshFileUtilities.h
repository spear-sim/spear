...
public:
    static IFileHandle* OpenFile(FString fileName, bool Read)
    {
        ...
        if (Read)
        {
            ...
            {
                throw std::runtime_error("Unable to find requested file " +
    ...
    static bool
    ReadNextLineTrimmingLeadingWhitespace(IFileHandle* fileHandle,
                                          FString& nextLine,
                                          int expectedLineLength = 128)
    {
        nextLine.Empty();
