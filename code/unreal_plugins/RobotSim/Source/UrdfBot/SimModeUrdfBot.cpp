...
void ASimModeUrdfBot::getExistingVehiclePawns(TArray<RobotSimVehicle*>& pawns) const
{
...
//
    ...
    for (auto const& vehicle_setting_pair : this->getSettings().vehicles) {
        ...
        if (vehicle_setting.auto_create && vehicle_setting.vehicle_type == RobotSimSettings::kVehicleTypeUrdfBot) {
    ...
    for (RobotSimVehicle* pawn : pawns) {
		...
		auto vehicle_sim_api = std::unique_ptr<RobotSimApi>(new RobotSimApi(params));
	...
	//URobotBlueprintLib::BindActionToKey("inputEventCycleCameraForward", EKeys::N, this, &ASimModeUrdfBot::cycleVisibleCameraForward);
	URobotBlueprintLib::BindActionToKey("inputEventCycleCameraBackward", EKeys::P, this, &ASimModeUrdfBot::cycleVisibleCameraBackward);
