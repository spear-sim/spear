...
AUrdfBotPawn::AUrdfBotPawn()
{
    ...
    try
    {
        ...
        {
            throw std::runtime_error("Invalid pawn_paths. Make sure that the UrdfFile exists");
        }
        ...
        for (auto kvp : parser.GetLinks())
        {
            ...
            if (visualSpecification != nullptr and visualSpecification->Geometry->GetGeometryType() == MESH)
            {
                linkGeometry = static_cast<UrdfMesh*>(visualSpecification->Geometry);
            }
            else if (collisionSpecification != nullptr and collisionSpecification->Geometry->GetGeometryType() == MESH)
            {
                linkGeometry = static_cast<UrdfMesh*>(visualSpecification->Geometry);
            }

            if (linkGeometry == nullptr or linkGeometry->FileType != UNREAL_MESH)
            {
                continue;
            }
...
void AUrdfBotPawn::InitializeForBeginPlay()
{
    ...
    RobotSimSettings::VehicleSetting* vehicleSetting = settings.vehicles["UrdfBot"].get();
    ...
    FString leftWheelJoint = vehicleSetting->controlSetting.LeftWheelJoint.c_str();
    if (leftWheelJoint.Len() > 0)
    {
        if (this->controlled_motion_components_.Contains(leftWheelJoint))
        {
            this->mLeftWheelJoint = static_cast<Motor*>(this->controlled_motion_components_[leftWheelJoint]);
            this->mLeftWheelJoint->SetDrive(0, 1000);
        }
    }
    // Set right wheel joint
    FString rightWheelJoint = vehicleSetting->controlSetting.RightWheelJoint.c_str();
    if (rightWheelJoint.Len() > 0)
    {
        if (this->controlled_motion_components_.Contains(rightWheelJoint))
        {
            this->mRightWheelJoint = static_cast<Motor*>(this->controlled_motion_components_[rightWheelJoint]);
            this->mRightWheelJoint->SetDrive(0, 1000);
        }
    }
    // set manipulator joint
    std::vector<std::string> manipulatorJoints = vehicleSetting->controlSetting.ManipulatorJoints;
    if (manipulatorJoints.size() > 0)
    {
        for (size_t i = 0; i < manipulatorJoints.size(); i++)
        {
            ...
            {
                this->mManipulatorJointList.Add(this->controlled_motion_components_[manipulatorJoint]);
                this->controlled_motion_components_[manipulatorJoint]->SetDrive(60000, 5);
            }
        }
    }
    // set end effector link
    FString endEffectorName = vehicleSetting->controlSetting.EndEffectorLink.c_str();
...
void AUrdfBotPawn::setupInputBindings()
{
    ...
    URobotBlueprintLib::BindAxisToKey(FInputAxisKeyMapping("onBrake", EKeys::BackSpace, 1), this, this, &AUrdfBotPawn::onBrake);
...
void AUrdfBotPawn::onGrabObject()
{
    ...
    FTransform result = getRelativePoseByLink(this->root_component_, this->mEndEffectorLink);
    ...
    const UrdfLinkSpecification& linkSpecification)
{
    ...
    if (visualGeometry == nullptr or collisionGeometry == nullptr)
    {
        return link;
    }
...
void AUrdfBotPawn::AttachChildren(AUrdfLink* parentLink,
                                  ...
                                  UrdfJointSpecification* jointSpecification)
{
    ...
    if (this->ConstraintNeedsControlledMotionComponent(*jointSpecification))
    {
        ...
        if (jointSpecification->Type == CONTINUOUS_TYPE and jointSpecification->Limit)
        {
...
FConstraintInstance AUrdfBotPawn::CreateConstraint(const UrdfJointSpecification& jointSpecification)
{
    ...
    case FLOATING_TYPE:
        ...
        break;
    case PRISMATIC_TYPE:
        range = (jointSpecification.Limit->Upper - jointSpecification.Limit->Lower) * this->world_scale_ * 0.5f * this->scale_factor_;

        constraintInstance.SetLinearXLimit(ELinearConstraintMotion::LCM_Limited, range);

        if (jointSpecification.Limit != nullptr and jointSpecification.Limit->Effort > 0)
        {
...
{
    if (spec.Type == PRISMATIC_TYPE or spec.Type == REVOLUTE_TYPE or spec.Type == CONTINUOUS_TYPE)
    {
        if (spec.Limit != nullptr)
        {
            return (spec.Limit->Effort > 0) and !(this->controlled_motion_components_.Contains(spec.Name));
        }
        else
        {
...
FVector AUrdfBotPawn::MoveChildLinkForLimitedXAxisMotion(AUrdfLink* parentLink,
                                                         AUrdfLink* childLink,
                                                         const UrdfJointSpecification& jointSpecification)
{
    ...
    if (jointSpecification.Limit == nullptr or jointSpecification.Limit->Lower < 0 - 1 or jointSpecification.Limit->Upper < 0)
    {
        return inverseTransform;
    }
    ...
    float lowerUU = jointSpecification.Limit->Lower * this->world_scale_ * this->scale_factor_;
    float upperUU = jointSpecification.Limit->Upper * this->world_scale_ * this->scale_factor_;
...
void AUrdfBotPawn::onBaseMove(float value)
{
    ...
    FVector leftTarget = this->mLeftWheelJoint->GetConstraintComponent()->ConstraintInstance.ProfileInstance.AngularDrive.AngularVelocityTarget;
    FVector rightTarget = this->mRightWheelJoint->GetConstraintComponent()->ConstraintInstance.ProfileInstance.AngularDrive.AngularVelocityTarget;

    float averageTargetVelocity = 0.5 * (leftTarget.X + rightTarget.X);
    float averageActualVelocity = 0.5 / (2 * PI) * (this->mLeftWheelJoint->GetMotionSpeed() + this->mRightWheelJoint->GetMotionSpeed());
    ...
    {
        if (this->mLeftWheelJoint->GetConstraintComponent()->ConstraintInstance.ProfileInstance.AngularDrive.IsVelocityDriveEnabled())
        {
...
void AUrdfBotPawn::onBaseRotate(float value)
{
    ...
    {
        this->mLeftWheelJoint->EnableDrive(true);
        FVector currentLeftTarget = this->mLeftWheelJoint->GetConstraintComponent()->ConstraintInstance.ProfileInstance.AngularDrive.AngularVelocityTarget;
        this->mLeftWheelJoint->SetDriveTargetVelocity(currentLeftTarget.X + value);

        this->mRightWheelJoint->EnableDrive(true);
        FVector currentRightTarget = this->mRightWheelJoint->GetConstraintComponent()->ConstraintInstance.ProfileInstance.AngularDrive.AngularVelocityTarget;
        this->mRightWheelJoint->SetDriveTargetVelocity(currentRightTarget.X - value);
    }
}
...
void AUrdfBotPawn::onBrake(float value)
{
    ...
    this->mRightWheelJoint->EnableDrive(true);
    this->mRightWheelJoint->SetDriveTargetVelocity(0);
    URobotBlueprintLib::LogMessage("onBrake", FString::SanitizeFloat(0), LogDebugLevel::Informational, 30);
}
...
void AUrdfBotPawn::onTest()
{
    ...
    for (auto& kvp : this->components_)
    {
        ...
        URobotBlueprintLib::LogMessage("link: " + kvp.Key, material->GetName() + " " + FString::SanitizeFloat(physical->Friction) + " " + FString::SanitizeFloat(physical->StaticFriction), LogDebugLevel::Informational, 30);
    }
    for (auto& kvp : this->constraints_)
    {
        URobotBlueprintLib::LogMessage("joint: " + kvp.Key, "", LogDebugLevel::Informational, 30);
    }
    UMeshComponent* mesh_root = root_component_->GetRootMesh();
    FTransform transform = mesh_root->GetRelativeTransform();

    URobotBlueprintLib::LogMessage(FString("rootMesh transform: \n"), transform.ToHumanReadableString(), LogDebugLevel::Informational, 30);
}
...
void AUrdfBotPawn::KeyBoardControl(FVector deltaPos, FVector deltaOri)
{
    ...
    FTransform currentPose = getRelativePoseByLink(this->root_component_, this->mEndEffectorLink);
    ...
    URobotBlueprintLib::LogMessage(FString("end effector target"), *temp.ToHumanReadableString(), LogDebugLevel::Informational, 30);
...
{
    ControlledMotionComponent* jointComponent = this->controlled_motion_components_[jointName];
    const FTransform& World2Base = jointComponent->GetParentLink()->GetTransform();
    FTransform temp(World2Base.GetRotation(), World2Base.GetTranslation(), FVector(1, 1, 1));
    AUrdfLink* childLink = jointComponent->GetActuatorLink();

    FVector parent2childPos = temp.InverseTransformPosition(childLink->GetActorLocation());
    FQuat parent2childOri = temp.InverseTransformRotation(childLink->GetActorRotation().Quaternion());
...
{
    if (this->components_.Contains(baseLinkName) and this->components_.Contains(TargetlinkName))
    {
        return getRelativePoseByLink(this->components_[baseLinkName], this->components_[TargetlinkName]);
    }
    else
    {
        return FTransform();
                                               ...
                                               AUrdfLink* targetLink)
{
    ...
    FTransform temp(World2Base.GetRotation(), World2Base.GetTranslation(), FVector(1, 1, 1));
    FVector base2targetPos = temp.InverseTransformPosition(targetLink->GetActorLocation());
    FQuat base2targetOri = temp.InverseTransformRotation(targetLink->GetActorRotation().Quaternion());
                                ...
                                TArray<FString>* jointList)
{
    ...
    {
        UPhysicsConstraintComponent* component = kvp.Value.Value;
        FString parentLinkName = component->ComponentName1.ComponentName.ToString();
        FString childLinkName = component->ComponentName2.ComponentName.ToString();
    ...
    while (currentLink && currentLink != this->root_component_)
    {
        ...
        {
            ControlledMotionComponent* joint = this->controlled_motion_components_[parentJointName];
