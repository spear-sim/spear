...
void FAUrdfLinkSecondaryTickFunction::ExecuteTick(
    float deltaTime,
    ELevelTick tickType,
    ENamedThreads::Type currentThread,
    const FGraphEventRef& myCompletionGraphEvent)
{
    if (this->target_ && !this->target_->IsPendingKill() &&
        !this->target_->IsUnreachable())
    {
        // For profiling
...
void AUrdfLink::BeginPlay()
{
    ...
    if (!IsTemplate() && this->secondaryComponentTick_.bCanEverTick)
    {
        ...
        this->secondaryComponentTick_.SetTickFunctionEnable(
            this->secondaryComponentTick_.bStartWithTickEnabled);
...
void AUrdfLink::TickPostPhysics(
    float deltaSeconds,
    ELevelTick tickType,
    FAUrdfLinkSecondaryTickFunction& thisTickFunction)
{
}

void AUrdfLink::NotifyHit(class UPrimitiveComponent* myComp,
                          class AActor* other,
                          ...
                          const FHitResult& hit)
{
    ...
    {
        this->owner_->NotifyHit(myComp, other, otherComp, bSelfMoved,
...
void AUrdfLink::GetReferenceFrameLocation(FVector& translation,
                                          FRotator& rotation)
{
...
void AUrdfLink::SetReferenceFrameLocation(const FVector& translation,
...
void AUrdfLink::AddForceSpecification(
    UrdfForceSpecification* forceSpecification)
{
    this->forceSpecifications_.Add(forceSpecification->Name,
                                   forceSpecification);
    this->lastSetMagnitudes_.Add(forceSpecification->Name, 0.0f);
    this->ComputeForces(0.0f, false);
}

void AUrdfLink::RemoveForceSpecification(
    UrdfForceSpecification* forceSpecification)
{
    ...
    if (this->forceSpecifications_.Contains(forceSpecificationName))
    {
        ...
        UrdfForceSpecification* val =
            this->forceSpecifications_[forceSpecificationName];
...
void AUrdfLink::SetForceMagnitude(FString forceName, float magnitude)
{
    ...
    {
        // TODO: Not sure if we should throw or just ignore requests for forces
        // that don't exist. For debugging purposes, we will throw. But there
        // might be valid use cases for silently ignoring them.
        throw std::runtime_error("Request for force specification '" +
                                 std::string(TCHAR_TO_UTF8(*forceName)) +
...
FVector AUrdfLink::WorldFromReferenceOffset(FVector referenceOffset,
                                            bool inputInMeters)
{
    // Get the location of the component
    FVector position = this->RootComponent->GetComponentLocation() -
                       this->visualOffsetTranslation_;
    FRotator rotation = this->RootComponent->GetComponentRotation() -
                        this->visualOffsetRotation_;

    // Transform the offset into the local frame
    float scale =
        (inputInMeters ? URobotBlueprintLib::GetWorldToMetersScale(this)
                       : 1.0f);
    FVector worldPosition =
        rotation.RotateVector(referenceOffset * scale) + position;
...
void AUrdfLink::ComputeForces(float deltaTime, bool inSubstep)
{
    ...
    for (auto kvp : this->forceSpecifications_)
    {
        ...
        case FORCE_ANGULAR:
            ...
            this->mesh_root_->AddTorque(angularForce->Magnitude * worldAxis *
                                        this->SiTorqueToUU_);
            break;
        case FORCE_LINEAR:
            linearForce = static_cast<UrdfLinearForceSpecification*>(force);
            this->mesh_root_->AddForceAtLocation(
                linearForce->Axis * linearForce->Magnitude * this->SiForceToUU_,
                this->WorldFromReferenceOffset(linearForce->ApplicationPoint,
                                               true));
            break;
        }
...
void AUrdfLink::UpdateKinematics(float dt)
{
    if (this->RootComponent != nullptr)
    {
        ...
        kinematics_.twist.linear =
            RobotSim::Vector3r(this->RootComponent->ComponentVelocity.X,
                               this->RootComponent->ComponentVelocity.Y,
                               this->RootComponent->ComponentVelocity.Z);
...
RobotSim::Pose AUrdfLink::GetPose()
{
    ...
    auto airRotation =
        RobotSim::Quaternionr(rotation.W, rotation.X, rotation.Y, rotation.Z);
...
void AUrdfLink::SetMeshFromStaticMeshComponent(
...
void AUrdfLink::SetMeshFromProceduralMeshComponent(
...
void AUrdfLink::InitPostSetMesh()
{
    ...
    // This can cause "bouncy" joints that should be stiff, and in extreme cases
    // can cause joints to settle in the wrong place. 10000 is an arbitrary
    // number that appears to work well without using too much CPU.
    int32 cnt = 100000;
    if (this->mesh_root_)
    {
