...
template <>
struct TStructOpsTypeTraits<FAUrdfLinkSecondaryTickFunction>
    : public TStructOpsTypeTraitsBase2<FAUrdfLinkSecondaryTickFunction>
{
    enum
    {
        WithCopy = false
...
public:
    ...
    void TickPostPhysics(float deltaSeconds,
                         ELevelTick tickType,
    ...
    void SetReferenceFrameLocation(const FVector& translation,
                                   const FRotator& rotation);
    FVector
    WorldFromReferenceOffset(FVector referenceOffset,
                             bool inputInMeters); // results are in UNREAL UNITS

    void RecordVisualOffset(FVector& translation, FRotator& rotation);

    // AUrdfLink now owns the ForceSpecification wrt memory allocation /
    // deallocation
    ...
    void SetMeshFromProceduralMeshComponent(
