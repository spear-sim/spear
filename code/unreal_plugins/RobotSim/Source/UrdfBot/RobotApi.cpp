...
//UrdfBotApi::UrdfBotApi(AUrdfBotPawn* pawn, const RobotSim::Kinematics::State* pawn_kinematics, const RobotSim::GeoPoint& home_geopoint, 
...
//    std::shared_ptr<RobotSim::SensorFactory> sensor_factory, const RobotSim::Environment& environment)
...
//{
//}
RobotApi::RobotApi(AUrdfBotPawn* pawn, const RobotSim::Kinematics::State* pawn_kinematics, const RobotSim::GeoPoint& home_geopoint,
	std::function<const RobotSim::Kinematics::State*(std::string)> state_provider_fxn, const RobotSim::RobotSimSettings::VehicleSetting* vehicle_setting,
...
void RobotApi::addLinearForce(const RobotApi::AddLinearForce& addedForce, const std::string& vehicle_name)
{
...
void RobotApi::addAngularForce(const RobotApi::AddAngularForce& addedForce, const std::string& vehicle_name)
{
	// TODO: For now, only use one vehicle_name.
	TMap<FString, AUrdfLink*> components = this->pawn_->GetLinkComponents();
	FString linkName = FString(addedForce.link_name.c_str());
...
void RobotApi::updateForceMagnitude(const UpdateForceMagnitude& updatedForce, const std::string& vehicle_name)
{
...
void RobotApi::updateControlledMotionComponentControlSignal(const UrdfBotControlledMotionComponentControlSignal& updated_control_signal, const std::string& vehicle_name)
{
...
void RobotApi::setAbsoluteJointPose(const UrdfBotControlledMotionComponentControlSignal& updated_control_signal, const std::string& vehicle_name)
{
	TMap<FString, ControlledMotionComponent*> components = this->pawn_->GetControlledMotionComponents();
	FString componentName = FString(updated_control_signal.component_name.c_str());
	if (components.Contains(componentName))
...
void RobotApi::setRelativeJointPose(const UrdfBotControlledMotionComponentControlSignal& updated_control_signal, const std::string& vehicle_name)
{
	TMap<FString, ControlledMotionComponent*> components = this->pawn_->GetControlledMotionComponents();
	FString componentName = FString(updated_control_signal.component_name.c_str());
	if (components.Contains(componentName))
...
void RobotApi::setBotInstantSpeed(const UrdfBotControlledMotionComponentControlSignal& updated_control_signal)
{
	TMap<FString, ControlledMotionComponent*> components = this->pawn_->GetBotMotionComponents();
	FString componentName = FString(updated_control_signal.component_name.c_str());
	if (components.Contains(componentName))
	{
		ControlledMotionComponent* component = components[componentName];
		float actual_speed = updated_control_signal.control_signal_values.at(std::string("Value"));
...
RobotSim::Momentums RobotApi::getBotInstantSpeed() const
{
	...
	*/
	...
	//TMap<FString, ControlledMotionComponent*>  motionComponents = this->pawn_->GetBotMotionComponents();
	//for (auto const& component : motionComponents)
	//{
	//	motion_component_speed[std::string(TCHAR_TO_UTF8(*component.Key))] = component.Value->GetMotorSpeed();
	//}
...
RobotSim::Momentums RobotApi::getLinkSpeed(const FString& linkName) const
{
	...
	if (botLink)
	{
		FVector  linearV = botLink->GetPhysicsAngularVelocityInRadians(); /* 每秒转动的弧度 */
...
const RobotApi::UrdfBotState RobotApi::getBotState(const std::string& vehicle_name)
{
	...
	for (auto &kvp : linkComponents)
	{
		...
		relativePose.position = RobotSim::Vector3r(relativeLocation[0], relativeLocation[1], relativeLocation[2]);
		relativePose.orientation = RobotSim::Quaternionr(relativeRotation.W, relativeRotation.X, relativeRotation.Y, relativeRotation.Z);

		relativeTwist.linear = RobotSim::Vector3r(relativeVelocity[0], relativeVelocity[1], relativeVelocity[2]);
		relativeTwist.angular = RobotSim::Vector3r(relativeAngularVelocity[0], relativeAngularVelocity[1], relativeAngularVelocity[2]);
...
std::vector<RobotApi::BotJoint> RobotApi::getAllJoints()
{
	...
	{
		TTuple<UrdfJointType, UPhysicsConstraintComponent*> joint = eachJoint.Value;
...
void RobotApi::setQueueTorque(const RobotSim::QueueVector3r& queueTorque)
{
	...
	//const std::vector<RobotSim::Vector3r>&   vecArray = queueTorque.mVector3rArray;
	//uint32   mapSize = motionComponents.Num();
	...
	//		element.Value->SetConstTorque(FVector(vec3r.x(), vec3r.y(), vec3r.z()));
	//	}
	//}
}
...
float RobotApi::getJointPose(const FString& jointName) const
{
	...
	/*TMap<FString, ControlledMotionComponent*>&    motionComponents = this->pawn_->GetControlledMotionComponents();
	if (motionComponents.Contains(jointName))
	{
		ControlledMotionComponent*  jointComponent = motionComponents[jointName];
		TMap<FString, FString> jointState = jointComponent->GetState();
		FString  actualPoint = jointState["ActualPoint"];
		actualPose = FCString::Atof(*actualPoint);
	}*/
...
float RobotApi::getJointVelocity(const FString& jointName) const
{
	...
	//	ControlledMotionComponent*  jointComponent = motionComponents[jointName];
	//	TMap<FString, FString> jointState = jointComponent->GetState();
	//	FString  actualPoint = jointState["ActualSpeed"];
	//	actualVelocity = FCString::Atof(*actualPoint);
	//}
