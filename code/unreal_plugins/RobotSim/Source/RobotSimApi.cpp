...
RobotSimApi::RobotSimApi(Params params)
	: RobotSimApiBase(params)
{
...
void RobotSimApi::updateRendering(float dt)
{
	...
	for (auto message : this->vehicle_api_messages_) {
		URobotBlueprintLib::LogMessage(FString(message.c_str()), TEXT(""), LogDebugLevel::Success, 30);
	}
	...
	try {
		...
		URobotBlueprintLib::LogMessage(FString(e.what()), TEXT(""), LogDebugLevel::Failure, 30);
	}
}
...
void RobotSimApi::createVehicleApi(AUrdfBotPawn* pawn, const RobotSim::GeoPoint& home_geopoint)
{
	...
	std::function<const RobotSim::Kinematics::State*(std::string)> state_provider_fxn =
		[&](std::string linkName)
	{
		FString linkNameFstr(linkName.c_str());
		return &(static_cast<AUrdfLink*>(pawn->GetLink(linkNameFstr))->GetKinematics());
	};
	this->vehicle_api_ = std::unique_ptr<RobotApi>(new RobotApi(pawn, getPawnKinematics(), home_geopoint, state_provider_fxn,
