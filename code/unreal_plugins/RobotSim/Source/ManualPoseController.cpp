...
void UManualPoseController::initializeForPlay()
{
    ...
    right_mapping_ =
        FInputAxisKeyMapping("inputManualArrowRight", EKeys::Right);
    forward_mapping_ = FInputAxisKeyMapping("inputManualForward", EKeys::Up);
    backward_mapping_ =
        FInputAxisKeyMapping("inputManualBackward", EKeys::Down);
    up_mapping_ = FInputAxisKeyMapping("inputManualArrowUp", EKeys::PageUp);
    down_mapping_ =
        FInputAxisKeyMapping("inputManualArrowDown", EKeys::PageDown);
    ...
    down_pitch_mapping_ =
        FInputAxisKeyMapping("inputManualDownPitch", EKeys::S);
...
void UManualPoseController::clearBindings()
{
    ...
    forward_binding_ = backward_binding_ = left_yaw_binding_ =
        up_pitch_binding_ = nullptr;
...
void UManualPoseController::updateActorPose(float dt)
{
    if (actor_ != nullptr)
    {
        ...
        actor_->SetActorLocationAndRotation(location + delta_position_,
                                            rotation + delta_rotation_);
        resetDelta();
    }
    else
    {
        URobotBlueprintLib::LogMessageString(
            "UManualPoseController::updateActorPose should not be called when "
            "actor is not set",
            "", LogDebugLevel::Failure);
    }
}

void UManualPoseController::getDeltaPose(FVector& delta_position,
                                         FRotator& delta_rotation) const
{
...
{
    if (left_binding_)
        URobotBlueprintLib::RemoveAxisBinding(left_mapping_, left_binding_,
                                              actor_);
    if (right_binding_)
        URobotBlueprintLib::RemoveAxisBinding(right_mapping_, right_binding_,
                                              actor_);
    if (forward_binding_)
        URobotBlueprintLib::RemoveAxisBinding(forward_mapping_,
                                              forward_binding_, actor_);
    if (backward_binding_)
        URobotBlueprintLib::RemoveAxisBinding(backward_mapping_,
                                              backward_binding_, actor_);
    if (up_binding_)
        URobotBlueprintLib::RemoveAxisBinding(up_mapping_, up_binding_, actor_);
    if (down_binding_)
        URobotBlueprintLib::RemoveAxisBinding(down_mapping_, down_binding_,
                                              actor_);
    if (left_yaw_binding_)
        URobotBlueprintLib::RemoveAxisBinding(left_yaw_mapping_,
                                              left_yaw_binding_, actor_);
    if (up_pitch_binding_)
        URobotBlueprintLib::RemoveAxisBinding(up_pitch_mapping_,
                                              up_pitch_binding_, actor_);
    if (right_yaw_binding_)
        URobotBlueprintLib::RemoveAxisBinding(right_yaw_mapping_,
                                              right_yaw_binding_, actor_);
    if (down_pitch_binding_)
        URobotBlueprintLib::RemoveAxisBinding(down_pitch_mapping_,
                                              down_pitch_binding_, actor_);
...
{
    URobotBlueprintLib::EnableInput(actor_);

    left_binding_ = &URobotBlueprintLib::BindAxisToKey(
        left_mapping_, actor_, this, &UManualPoseController::inputManualLeft);
    right_binding_ = &URobotBlueprintLib::BindAxisToKey(
        right_mapping_, actor_, this, &UManualPoseController::inputManualRight);
    forward_binding_ = &URobotBlueprintLib::BindAxisToKey(
        forward_mapping_, actor_, this,
        &UManualPoseController::inputManualForward);
    backward_binding_ = &URobotBlueprintLib::BindAxisToKey(
        backward_mapping_, actor_, this,
        &UManualPoseController::inputManualBackward);
    up_binding_ = &URobotBlueprintLib::BindAxisToKey(
        up_mapping_, actor_, this, &UManualPoseController::inputManualMoveUp);
    down_binding_ = &URobotBlueprintLib::BindAxisToKey(
        down_mapping_, actor_, this, &UManualPoseController::inputManualDown);
    left_yaw_binding_ = &URobotBlueprintLib::BindAxisToKey(
        left_yaw_mapping_, actor_, this,
        &UManualPoseController::inputManualLeftYaw);
    up_pitch_binding_ = &URobotBlueprintLib::BindAxisToKey(
        up_pitch_mapping_, actor_, this,
        &UManualPoseController::inputManualUpPitch);
    right_yaw_binding_ = &URobotBlueprintLib::BindAxisToKey(
        right_yaw_mapping_, actor_, this,
        &UManualPoseController::inputManualRightYaw);
    down_pitch_binding_ = &URobotBlueprintLib::BindAxisToKey(
        down_pitch_mapping_, actor_, this,
...
void UManualPoseController::updateDeltaPosition(float dt)
{
    ...
    if (!FMath::IsNearlyZero(input.SizeSquared()))
    {
        ...
        delta_position_ +=
            actor_->GetActorRotation().RotateVector(last_velocity_ * dt);
    }
    else
    {
