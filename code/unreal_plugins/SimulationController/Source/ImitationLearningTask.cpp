...
bool ImitationLearningTask::sampleTrajectoryToRandomTarget()
{
    ...
    while (number_iterations < Config::getValue<int>({"SIMULATION_CONTROLLER", "NAVIGATION", "MAX_ITER_REPLAN"})) // Try to generate interesting trajectories with multiple waypoints
    {
        ...
        ASSERT(nav_sys_->GetRandomReachablePointInRadius(agent_initial_position_.at(trajectory_index_), Config::getValue<float>({"SIMULATION_CONTROLLER", "NAVIGATION", "TARGET_RADIUS"}), target_location));

        // Update relative position between the agent and its new target:
        relative_position_to_target.X = (target_location.Location - agent_initial_position_.at(trajectory_index_)).X;
        relative_position_to_target.Y = (target_location.Location - agent_initial_position_.at(trajectory_index_)).Y;

        // Update navigation query with the new target:
        nav_query = FPathFindingQuery(agent_actor_, *nav_data_, agent_initial_position_.at(trajectory_index_), target_location.Location);
    ...
    agent_goal_position_.at(trajectory_index_) = best_target_location.Location;
    target_point_generated_ = true;
