...
std::map<std::string, Box> DebugAgentController::getObservationSpace() const
{
    ...
    box.low = 0;
    box.high = 255;
    box.shape = {Config::getValue<unsigned long>({"DEBUG_PROJECT", "IMAGE_HEIGHT"}), Config::getValue<unsigned long>({"DEBUG_PROJECT", "IMAGE_WIDTH"})};
    box.dtype = DataType::UInteger8;
    observation_space["camera_1_image"] = box;
    ...
    box.dtype = DataType::UInteger8;
    observation_space["camera_2_image"] = box;
