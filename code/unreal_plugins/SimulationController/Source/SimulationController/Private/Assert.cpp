...
AssertAction::AssertAction PPK_ASSERT_CALL
_defaultHandler(const char* file,
                int line,
                const char* function,
                const char* expression,
                int level,
                const char* message)
{
    ...
    formatLevel(level, expression, stderr,
                reinterpret_cast<printHandler>(print));
    ...
    defined(PPK_ASSERT_DEFAULT_HANDLER_STDIN)
        for (;;)
        {
            ...
            fprintf(stderr,
                    "Press (I)gnore / Ignore (A)ll / (D)ebug / A(b)ort: ");
#else
            ...
            const char* message)
{
    ...
    throwException(ppk::assert::AssertionException(file, line, function,
                                                   expression, message));
}
} // namespace
...
AssertionException::AssertionException(const char* file,
                                       ...
                                       const char* expression,
                                       const char* message)
    : _file(file), _line(line), _function(function), _expression(expression),
      _heap(PPK_ASSERT_NULLPTR)
{
    if (!message)
    {
    ...
    if (length < size) // message is short enough for the stack buffer
    {
        ...
        memset(_stack + length, 0,
               sizeof(char) * (size - length)); // pad with 0
    }
    else // allocate storage on the heap
    {
        _heap =
            static_cast<char*>(PPK_ASSERT_MALLOC(sizeof(char) * (length + 1)));

        if (!_heap) // allocation failed
        {
            memcpy(_stack, message,
                   sizeof(char) * (size - 1)); // stack fallback, truncate :/
            _stack[size - 1] = 0;
        }
        else
        {
...
AssertionException::AssertionException(const AssertionException& rhs)
    : _file(rhs._file), _line(rhs._line), _function(rhs._function),
...
AssertionException::~AssertionException() PPK_ASSERT_EXCEPTION_NO_THROW
{
    ...
    _heap =
        PPK_ASSERT_NULLPTR; // in case the exception object is destroyed twice
    _stack[size - 1] = 0;
}
...
AssertionException& AssertionException::operator=(const AssertionException& rhs)
{
    ...
    else // allocate storage on the heap
    {
        if (_stack[size - 1])
        {
            ...
            if (length <= _length)
            {
                memcpy(_heap, message,
                       sizeof(char) * _length); // copy the message, pad with 0
                return *this;
            }
            else
            {
                                                        ...
                                                        ...)
{
    ...
    AssertAction::AssertAction action =
        _handler(file, line, function, expression, level, message);
