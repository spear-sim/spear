...
CameraAgentController::CameraAgentController(UWorld* world)
{
    ...
    // create render passes based on image_type
    ASSERT(Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_TYPE" }) == "rgb" || Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_TYPE" }) == "seg");

    std::vector<std::string> render_passes;
    if (Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_TYPE" }) == "rgb") {
        render_passes.emplace_back("final_color");
    }
    else if (Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_TYPE" }) == "seg") {
        render_passes.emplace_back("segmentation");
    }

    camera_sensor_ = std::make_unique<CameraSensor>(camera_actor_, render_passes,
...
std::map<std::string, Box> CameraAgentController::getActionSpace() const
{
    ...
    ASSERT(Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "ACTION_MODE" }) == "sample_images" || Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "ACTION_MODE" }) == "replay_sampled_images");

    if (Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "ACTION_MODE" }) == "sample_images") {
        box.low = std::numeric_limits<float>::lowest();
        box.high = std::numeric_limits<float>::max();
        box.shape = { 1 };
        box.dtype = DataType::Float32;
    ...
    else if (Config::getValue<std::string>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "ACTION_MODE" }) == "replay_sampled_images") {
        box.low = std::numeric_limits<float>::lowest();
        box.high = std::numeric_limits<float>::max();
        box.shape = { 3 };
        box.dtype = DataType::Float32;
...
std::map<std::string, std::vector<uint8_t>> CameraAgentController::getObservation() const
{
    ...
    std::vector<uint8_t> image(Config::getValue<unsigned long>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_HEIGHT" }) * Config::getValue<unsigned long>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "IMAGE_WIDTH" }) * 3);
...
void CameraAgentController::buildNavMesh()
{
    ...
    //FBox worldBox(box.Min, FVector(box.Max.X, box.Max.Y, box.Min.Z + Config::getValue<float>({ "SIMULATION_CONTROLLER", "CAMERA_AGENT_CONTROLLER", "NAVMESH", "NAV_BOUND_BOX_HEIGHT_ABOVE_MIN" })));
