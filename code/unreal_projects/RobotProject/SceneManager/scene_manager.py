...
def get_scene_config(virtualworld_id, version):
    scene_meta_url = CDN_API +'scenes/'+virtualworld_id+'/'+version+'/ConfigMeta_{}.json'.format(virtualworld_id)
    scene_meta_local = os.path.join(TEMP_FOLDER,virtualworld_id,'ConfigMeta_{}.json'.format(virtualworld_id))
...
def download_scenes(virtualworld_id, version, scene_config_data, is_force_update):
    ...
    local_version_info = os.path.join(local_version_info_folder,version_scene_info_name)
    remote_version_info_url = CDN_API+'scenes/'+virtualworld_id+'/'+version+'/'+version_scene_info_name
    ...
    anim_url = CDN_API+'scenes/'+virtualworld_id+'/'+version+'/'+scene_config_data['animation']
    anim_local = os.path.join(TEMP_FOLDER,scene_config_data['animation'])
    anim_dst = os.path.join(os.path.dirname(__file__),'../Content/Scene/',scene_config_data['animation'].replace('.zip',''))
    is_anim_ok = deal_scene_file(anim_url, anim_local, anim_dst)

    arch_url = CDN_API+'scenes/'+virtualworld_id+'/'+version+'/'+scene_config_data['architecture']
    arch_local = os.path.join(TEMP_FOLDER,scene_config_data['architecture'])
    arch_dst = os.path.join(os.path.dirname(__file__),'../Content/Scene/Meshes',scene_config_data['architecture'].replace('.zip',''))
    is_arch_ok = deal_scene_file(arch_url, arch_local, arch_dst)

    mat_url = CDN_API+'scenes/'+virtualworld_id+'/'+version+'/'+scene_config_data['materialinst']
    mat_local = os.path.join(TEMP_FOLDER,scene_config_data['materialinst'])
    mat_dst = os.path.join(os.path.dirname(__file__),'../Content/Scene/',scene_config_data['materialinst'].replace('.zip',''))
    ...
    download_file_from_url(CDN_API+'scenes/'+virtualworld_id+'/'+version+'/'+scene_config_data['map'], umap_local)
    is_umap_ok = False
...
def download_assets(virtualworld_id, version, is_down_ddc, scene_config_data, is_force_update):
    ...
    phys_furniture_version_info_folder = os.path.join(local_version_info_folder,'phys_furniture')
    ...
    for materialid in scene_config_data['material']:
        dic_curr = {}
        dic_curr['asset_url'] = CDN_API+'assets/material/'+materialid+'/'+version+'/'+materialid+'.zip'
        dic_curr['asset_local'] = os.path.join(TEMP_FOLDER, materialid+'.zip')
        dic_curr['asset_dst'] = os.path.join(os.path.dirname(__file__),'../Content/Scene/Materials',materialid)
        dic_curr['version_info_url'] = CDN_API+'assets/material/'+materialid+'/'+version+'/'+'version_info_'+materialid+'.json'
        dic_curr['version_info_local'] = os.path.join(TEMP_FOLDER, 'version_info_'+materialid+'.json')
        dic_curr['version_info_dst'] = os.path.join(mat_version_info_folder,'version_info_'+materialid+'.json')
    ...
    for meshid in scene_config_data['mesh']:
        dic_curr = {}
        dic_curr['asset_url'] = CDN_API+'assets/furniture/'+meshid+'/'+version+'/'+meshid+'.zip'
        dic_curr['asset_local'] = os.path.join(TEMP_FOLDER, meshid+'.zip')
        dic_curr['asset_dst'] = os.path.join(os.path.dirname(__file__),'../Content/Scene/Meshes/Furniture',meshid)
        dic_curr['version_info_url'] = CDN_API+'assets/furniture/'+meshid+'/'+version+'/'+'version_info_'+meshid+'.json'
        dic_curr['version_info_local'] = os.path.join(TEMP_FOLDER, 'version_info_'+meshid+'.json')
        dic_curr['version_info_dst'] = os.path.join(phys_furniture_version_info_folder,'version_info_'+meshid+'.json')
    ...
    for meshid in scene_config_data['mesh']:
        dic_curr = {}
        dic_curr['asset_url'] = CDN_API+'assets/phys_furniture/'+meshid+'/'+version+'/'+meshid+'.zip'
        dic_curr['asset_local'] = os.path.join(TEMP_FOLDER, meshid+'.zip')
        dic_curr['asset_dst'] = os.path.join(os.path.dirname(__file__),'../Content/Scene/Meshes/PhysicalFurniture',meshid)
        dic_curr['version_info_url'] = CDN_API+'assets/phys_furniture/'+meshid+'/'+version+'/'+'version_info_'+meshid+'.json'
        dic_curr['version_info_local'] = os.path.join(TEMP_FOLDER, 'version_info_'+meshid+'.json')
        dic_curr['version_info_dst'] = os.path.join(furniture_version_info_folder,'version_info_'+meshid+'.json')
    ...
    if is_down_ddc:
        ...
        for ddc_path in scene_config_data['DDC']:
            ...
            dic_curr = {}
            dic_curr['asset_url'] = CDN_API+'assets/ddc/'+ddc_only_path+'/'+version+'/'+ ddc_name
            dic_curr['asset_local'] = os.path.join(os.path.dirname(__file__),'../DerivedDataCache',ddc_path)
            if not os.path.exists(os.path.dirname(dic_curr['asset_local'])):
                try:
                    os.makedirs(dic_curr['asset_local'])
                except:
                    pass
            dic_curr['asset_dst'] = os.path.join(os.path.dirname(__file__),'../DerivedDataCache',ddc_path)
            dic_curr['version_info_url'] = CDN_API+'assets/ddc/'+ddc_only_path+'/'+version+'/'+'version_info_'+ddc_name.replace('.udd','')+'.json'
            dic_curr['version_info_local'] = os.path.join(TEMP_FOLDER, 'version_info_'+ddc_name.replace('.udd','')+'.json')
            dic_curr['version_info_dst'] = os.path.join(ddc_version_info_folder,'version_info_'+ddc_name.replace('.udd','')+'.json')
...
def download_single_virtualworld(virtualworld_id, version, is_down_ddc, is_force_update):
    # get scene meta
    scene_config_data = get_scene_config(virtualworld_id, version)
    # download assets
    is_assets_ready = download_assets(virtualworld_id, version, is_down_ddc, scene_config_data, is_force_update)
    is_scene_ready = False
    # download scenes
    if is_assets_ready:
        is_scene_ready = download_scenes(virtualworld_id, version, scene_config_data, is_force_update)
    ...
    if is_assets_ready and is_scene_ready:
        ...
        with open(log,'w') as f:
            f.writelines('Please update {} again. assset is {} and scene is {}'.format(virtualworld_id, is_assets_ready, is_scene_ready))
        print('Please update {} again. assset is {} and scene is {}'.format(virtualworld_id, is_assets_ready, is_scene_ready))
        return False

def print_help():
    print('''scene_manager.py -i <option> -v <necessary> -d <option> -f <option>
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"h:i:v:d:f:",["help=","infile=","version=","downDDC=","forceUpade="])
    ...
    if version == '':
            print('Error: set version correct. please select version from SceneManage/Data/dataset-repo-update.log')
            print_help()
        ...
        if os.path.exists(virtualworld_ids_file):
            with open(virtualworld_ids_file) as f:
                for id in json.load(f):
                    download_single_virtualworld(id, version, is_down_ddc, is_force_update)
    else:
        download_single_virtualworld(virtualworld_id, version, is_down_ddc, is_force_update)
