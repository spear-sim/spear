...
if __name__ == "__main__":
    ...
    parser.add_argument("--scenes_path", type=str, default="")
    ...
    for scene in chosen_scenes:
        ...
        if not os.path.exists(os.path.join(os.path.dirname(os.path.realpath(__file__)), f"data/scene_{scene}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.ACTION_MODE}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.IMAGE_TYPE}")):
            os.makedirs(os.path.join(os.path.dirname(os.path.realpath(__file__)), f"data/scene_{scene}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.ACTION_MODE}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.IMAGE_TYPE}"))
            print(f"collecting images for scene {scene}")
        if not os.path.exists(os.path.join(os.path.dirname(os.path.realpath(__file__)), f"data/scene_{scene}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.ACTION_MODE}/depth")):
            os.makedirs(os.path.join(os.path.dirname(os.path.realpath(__file__)), f"data/scene_{scene}/{config.SIMULATION_CONTROLLER.SLAM_DATASET_AGENT_CONTROLLER.ACTION_MODE}/depth"))
        ...
        with open(pose_info_file, 'r') as f:
            ...
            for row in list(csv_reader)[1:]:
                ...
                #hdf5 file save
                with h5py.File(output_path + "/../depth/" +f"{count}.hdf5", 'w') as hf:
                    hf.create_dataset(f"depth_{count}",  data=obs["visual_observation_depth"], dtype=obs["visual_observation_depth"].dtype, shape=obs["visual_observation_depth"].shape)

                #csv file save
                # np.savetxt(output_path + "/../depth/" +f"{count}.csv", obs["visual_observation_depth"], delimiter=",")
