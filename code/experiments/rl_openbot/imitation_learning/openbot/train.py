...
class Hyperparameters:
    ...
    @classmethod
    def parse(cls, name):
        m = re.match(r".*_((cil|pilot)_.+)_lr(\d+.\d+)_bz(\d+)(_bn)?", name)
...
def process_data(tr: Training, redo_matching=False, remove_zeros=True):
    ...
    # 1ms
    max_offset = 1e3
    train_frames = associate_frames.match_frame_ctrl_goal(
        tr.train_data_dir,
        tr.train_datasets,
        max_offset,
    ...
    test_frames = associate_frames.match_frame_ctrl_goal(
        tr.test_data_dir,
        tr.test_datasets,
        max_offset,
        redo_matching=redo_matching,
...
def load_data(tr: Training, verbose=0):
    ...
    def process_train_path(file_path):
        goal, label = train_data.get_label(
...
def do_training(tr: Training, callback: tf.keras.callbacks.Callback, verbose=0):
    ...
    if tr.hyperparameters.USE_LAST:
        try:
            ...
            last_checkpoint = os.path.join(tr.checkpoint_path, sorted(dirs)[-1])
            ...
            INITIAL_EPOCH = int(log_file.readlines()[-1].split(",")[0]) + 1
            log_file.close()
