...
def compare_tf_tflite(model, tflite_model, img=None, goal=None):
    ...
    for input_detail in input_details:
        if "img_input" in input_detail["name"]:
            ...
            if goal is None:
                input_data["goal_input"] = np.array(
                    np.random.random_sample(input_detail["shape"]), dtype=np.float32
                )
            else:
                print(goal)
                input_data[input_detail["name"]] = goal
            interpreter.set_tensor(input_detail["index"], input_data["goal_input"])
        else:
    ...
    tf_results = model.predict(
        (tf.constant(input_data["img_input"]), tf.constant(input_data["goal_input"]))
    )
    print("tf:", tf_results)
