...
for folder in os.listdir(f"dataset/uploaded"): # for each environment folders
    ...
    for dataFolder in os.listdir(path): # for each of the 100 data folders 
        ...
        try:
            ...
            vehicleFacingGoal = math.isclose(array_goal[numIter - 10][3], 1.0, abs_tol = 0.2) 
            
            # Are the last 10 control iterations close to zero ? 
            finalControlIsZero = math.isclose(np.mean(array_ctrl[numIter - 11 :, 1]) + np.mean(array_ctrl[numIter - 11 : , 2]), 0.0, abs_tol = 0.01)
            ...
            print(f"Error: One of the files in {folderName}/sensor_data appears to be corrupted.")
            print(f"Removing {folderName}...")
            shutil.rmtree(folderName)
        
        if not f_goal_valid:
            print(f"File {folderName}/sensor_data/goalLog.txt does not appear to contain relevant data.")
            print(f"Removing {folderName}...")
            shutil.rmtree(folderName)
