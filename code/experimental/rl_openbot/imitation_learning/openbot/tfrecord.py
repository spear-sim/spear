...
def load_labels(data_dir, datasets):
    """Returns a dictionary of matched images path[string] and actions tuple (left[int], right[int], dist[float],sinYaw[float],cosYaw[float])."""
    corpus = []
    for dataset in datasets:
        ...
        for folder in dataset_folders:
            ...
            with open(
                os.path.join(sensor_data_dir, "matched_frame_ctrl_goal_processed.txt")
            ) as f_input:
                ...
                # Tuples containing id: framepath and label: left,right,dist,sinYaw,cosYaw
...
def convert_dataset(
    ...
    # match frames.
    max_offset = 1e3  # 1ms
    frames = associate_frames.match_frame_ctrl_goal(
        data_dir,
        datasets,
        max_offset,
    ...
    with tf.io.TFRecordWriter(tfrecords_dir + "/" + tfrecords_name) as writer:
        for image_path, ctrl_goal in samples.items():
            try:
                image = tf.io.decode_jpeg(tf.io.read_file(image_path))
                example = tfrecord_utils.create_example(image, image_path, ctrl_goal)
