...
void ADummyActor::BeginPlay()
{
    ...
    std::ofstream myfile;
    std::string file = "E:/new_isim_images/NavMeshDebugProject/poses_for_debug.csv";
...
void ADummyActor::rebuildNavSystem()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("navsys autoupdatenabled %d"), nav_sys->GetIsAutoUpdateEnabled());
    ...
    //nav_modifier_volume->SetActorLocation(worldBox.GetCenter(), false);          // Place the navmesh at the center of the map
    //nav_modifier_volume->SetActorRelativeScale3D(0.9 * worldBox.GetSize() / 200.f);   // Rescale the navmesh
    ...
    */
    ...
    ANavModifierVolume* nav_modifier_volume = Cast<ANavModifierVolume>(GetWorld()->SpawnActor<ANavModifierVolume>());
    //nav_modifier_volume->SetAreaClass(nav_mesh_bounds->areaclass);
    nav_modifier_volume->GetRootComponent()->SetMobility(EComponentMobility::Movable);
    nav_modifier_volume->SetActorLocation(worldBox.GetCenter(), false);          // Place the navmesh at the center of the map
    nav_modifier_volume->SetActorRelativeScale3D(worldBox.GetSize() / 200.f);   // Rescale the navmesh
    nav_modifier_volume->GetRootComponent()->UpdateBounds();
    nav_modifier_volume->GetRootComponent()->SetMobility(EComponentMobility::Static);
    nav_modifier_volume->RebuildNavigationData();
    ...
    FBox nav_mesh_box = nav_mesh_bounds->GetComponentsBoundingBox(true, true);
    FBox nav_modifier_box = nav_modifier_volume->GetComponentsBoundingBox(true, true);

    UE_LOG(LogTemp, Warning, TEXT("worldbox center (X,Y,Z) : (%f, %f, %f)"), worldBox.GetCenter().X, worldBox.GetCenter().Y, worldBox.GetCenter().Z);

    UE_LOG(LogTemp, Warning, TEXT("nav_mesh_bounds (X,Y,Z): (%f, %f, %f)"), nav_mesh_bounds->GetActorLocation().X, nav_mesh_bounds->GetActorLocation().Y, nav_mesh_bounds->GetActorLocation().Z);
    UE_LOG(LogTemp, Warning, TEXT("nav_mesh_box_min (X,Y,Z): (%f, %f, %f)"), nav_mesh_box.Min.X, nav_mesh_box.Min.Y, nav_mesh_box.Min.Z);
    UE_LOG(LogTemp, Warning, TEXT("nav_mesh_box_max (X,Y,Z): (%f, %f, %f)"), nav_mesh_box.Max.X, nav_mesh_box.Max.Y, nav_mesh_box.Max.Z);

    UE_LOG(LogTemp, Warning, TEXT("nav_modifier_volume (X,Y,Z): (%f, %f, %f)"), nav_modifier_volume->GetActorLocation().X, nav_modifier_volume->GetActorLocation().Y, nav_modifier_volume->GetActorLocation().Z);
    UE_LOG(LogTemp, Warning, TEXT("nav_modifier_box_min (X,Y,Z): (%f, %f, %f)"), nav_modifier_box.Min.X, nav_modifier_box.Min.Y, nav_modifier_box.Min.Z);
    UE_LOG(LogTemp, Warning, TEXT("nav_modifier_box_max (X,Y,Z): (%f, %f, %f)"), nav_modifier_box.Max.X, nav_modifier_box.Max.Y, nav_modifier_box.Max.Z);
