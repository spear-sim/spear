...
`CLAUDE.md` and `.cursor/rules/local-style.mdc` must always be kept in sync.
...
Shared documentation lives in `docs/` so both files can point to the same source of truth:

- `docs/agents.md` — SPEAR programming model (read always).
- `docs/agents_mcp_usage.md` — MCP tool usage, API patterns, and rendering. Read once at the start of a session that uses SPEAR MCP tools.
- `docs/agents_style_guide.md` — code style conventions. Read when writing or reviewing code that will be checked in.
