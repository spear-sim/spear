...
private:
    ...
    static auto wrapFuncToExecuteInWorkQueueBlockingImpl(WorkQueue& work_queue, TFunc&& func, const FuncInfo<TReturn(*)(TArgs&...)>& fi)
    {
        ...
        // accessible after this wrapFuncToExecuteInWorkQueueBlockingImpl(...) function returns.
        ...
        return [&work_queue, func](TArgs&... args) -> TReturn {
            ...
            auto task = CopyConstructiblePackagedTask(
                [func, args...]() -> TReturn {
                    return func(args...);
                });
    ...
    template <typename TFunc>
    struct CopyConstructiblePackagedTask : std::packaged_task<std::invoke_result_t<TFunc>()>
    {
        CopyConstructiblePackagedTask(TFunc&& func) : std::packaged_task<std::invoke_result_t<TFunc>()>(std::forward<TFunc>(func)) {};
    };
