...
#include "SpEngine/Legacy/PointGoalNavTask.h"
...
PointGoalNavTask::PointGoalNavTask(UWorld* world)
{
    ...
    actor_spawn_parameters.Name = Unreal::toFName(Config::get<std::string>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_ACTOR_NAME"));
    ...
    FVector scale =
        FVector(
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_SCALE"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_SCALE"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_SCALE"));
    goal_actor_->SetActorScale3D(scale);

    // Load mesh and material
    auto goal_mesh = LoadObject<UStaticMesh>(nullptr, *Unreal::toFString(Config::get<std::string>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_MESH")));
    SP_ASSERT(goal_mesh);
    auto goal_material = LoadObject<UMaterial>(nullptr, *Unreal::toFString(Config::get<std::string>("SP_ENGINE.POINT_GOAL_NAV_TASK.GOAL_MATERIAL")));
    ...
    minstd_rand_ = std::minstd_rand(Config::get<int>("SP_ENGINE.POINT_GOAL_NAV_TASK.RANDOM_SEED"));
    hit_goal_ = false;
    hit_obstacle_ = false;
}
...
{
    agent_actor_ = Unreal::findActorByName(world, Config::get<std::string>("SP_ENGINE.POINT_GOAL_NAV_TASK.AGENT_ACTOR_NAME"));
    SP_ASSERT(agent_actor_);

    bool return_null_if_not_found = false;
    obstacle_ignore_actors_ = Unreal::findActorsByName(
        world, Config::get<std::vector<std::string>>("SP_ENGINE.POINT_GOAL_NAV_TASK.OBSTACLE_IGNORE_ACTOR_NAMES"), return_null_if_not_found);
...
float PointGoalNavTask::getReward() const
{
    ...
    if (hit_goal_) {
        reward = Config::get<float>("SP_ENGINE.POINT_GOAL_NAV_TASK.REWARD.HIT_GOAL");
    } else if (hit_obstacle_) {
        reward = Config::get<float>("SP_ENGINE.POINT_GOAL_NAV_TASK.REWARD.HIT_OBSTACLE");
    } else {
...
void PointGoalNavTask::reset()
{
    ...
    while ((agent_position - goal_position).Size() < Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.SPAWN_DISTANCE_THRESHOLD")) {

        std::uniform_real_distribution distribution_agent_position_x(
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.AGENT_LOCATION_X_MIN"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.AGENT_LOCATION_X_MAX"));
        std::uniform_real_distribution distribution_agent_position_y(
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.AGENT_LOCATION_Y_MIN"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.AGENT_LOCATION_Y_MAX"));
        std::uniform_real_distribution distribution_goal_position_x(
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.GOAL_LOCATION_X_MIN"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.GOAL_LOCATION_X_MAX"));
        std::uniform_real_distribution distribution_goal_position_y(
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.GOAL_LOCATION_Y_MIN"),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.GOAL_LOCATION_Y_MAX"));

        agent_position = FVector(
            distribution_agent_position_x(minstd_rand_),
            distribution_agent_position_y(minstd_rand_),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.AGENT_LOCATION_Z"));
        goal_position = FVector(
            distribution_goal_position_x(minstd_rand_),
            distribution_goal_position_y(minstd_rand_),
            Config::get<double>("SP_ENGINE.POINT_GOAL_NAV_TASK.EPISODE_BEGIN.GOAL_LOCATION_Z"));
    }
