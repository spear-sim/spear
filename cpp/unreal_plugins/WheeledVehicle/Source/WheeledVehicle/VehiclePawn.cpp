...
{
    std::cout << "[SPEAR | VehiclePawn.cpp] AVehiclePawn::AVehiclePawn" << std::endl;
...
void AVehiclePawn::setDriveTorquesFromDutyCycle()
{
    //UE_LOG(LogTemp, Warning, TEXT("VehiclePawn.cpp::setDriveTorquesFromDutyCycle(), duty_cycle_(0) = %f"), duty_cycle_(0));
    //UE_LOG(LogTemp, Warning, TEXT("VehiclePawn.cpp::setDriveTorquesFromDutyCycle(), duty_cycle_(1) = %f"), duty_cycle_(1));
    //UE_LOG(LogTemp, Warning, TEXT("VehiclePawn.cpp::setDriveTorquesFromDutyCycle(), duty_cycle_(2) = %f"), duty_cycle_(2));
    //UE_LOG(LogTemp, Warning, TEXT("VehiclePawn.cpp::setDriveTorquesFromDutyCycle(), duty_cycle_(3) = %f"), duty_cycle_(3));
    ...
    auto motor_velocity_constant = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.MOTOR_VELOCITY_CONSTANT"); // Motor torque constant in [N.m/A]
    auto gear_ratio = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.GEAR_RATIO");                           // Gear ratio of the WheeledVehicle motors
    auto motor_torque_constant = 1.f / motor_velocity_constant;                                        // Motor torque constant in [rad/s/V]

    auto battery_voltage = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.BATTERY_VOLTAGE");         // Voltage of the battery powering the WheeledVehicle [V]
    auto control_dead_zone = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.CONTROL_DEAD_ZONE");     // Absolute duty cycle (in the ACTION_SCALE range)
    // below which a command does not produces any torque
    // on the vehicle
    auto motor_torque_max = Config::get<float>("WHEELED_VEHICLE.VEHICLE_COMPONENT.MOTOR_TORQUE_MAX");             // Motor maximal torque [N.m]
    auto electrical_resistance = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.ELECTRICAL_RESISTANCE");   // Motor winding electrical resistance [Ohms]
    auto electrical_inductance = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.ELECTRICAL_INDUCTANCE");   // Motor winding electrical inductance [Henry]

    // Speed multiplier defined in the WheeledVehicle to map a [-1,1] action to a suitable command to
    // be processed by the low-level microcontroller. For more details, feel free to check the
    // "speedMultiplier" command in the WheeledVehicle code.
    // https://github.com/isl-org/WheeledVehicle/blob/master/android/app/src/main/java/org/openbot/vehicle/Vehicle.java#L375

    auto action_scale = Config::get<float>("WHEELED_VEHICLE.VEHICLE_PAWN.ACTION_SCALE");
    ...
    //UE_LOG(LogTemp, Warning, TEXT("AVehiclePawn.cpp::setDriveTorquesFromDutyCycle(), setting wheel_torque(0) = %f"), wheel_torque(0));
    //UE_LOG(LogTemp, Warning, TEXT("AVehiclePawn.cpp::setDriveTorquesFromDutyCycle(), setting wheel_torque(1) = %f"), wheel_torque(1));
    //UE_LOG(LogTemp, Warning, TEXT("AVehiclePawn.cpp::setDriveTorquesFromDutyCycle(), setting wheel_torque(2) = %f"), wheel_torque(2));
    //UE_LOG(LogTemp, Warning, TEXT("AVehiclePawn.cpp::setDriveTorquesFromDutyCycle(), setting wheel_torque(3) = %f"), wheel_torque(3));
...
Eigen::VectorXf AVehiclePawn::rpmToRadSec(Eigen::VectorXf rpm)
{
    return rpm * PI / 30.f;
}

// rad/s to rev per minute
Eigen::VectorXf AVehiclePawn::radSecToRpm(Eigen::VectorXf omega)
{
