...
SphereAgent::SphereAgent(UWorld* world)
{
    ...
    std::string spawn_mode = Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_MODE");
    if (spawn_mode == "specify_existing_actor") {
        AActor* spawn_actor = Unreal::findActorByName(world, Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_ACTOR_NAME"));
        ...
        spawn_location = FVector(
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_LOCATION_X"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_LOCATION_Y"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_LOCATION_Z"));
        spawn_rotation = FRotator(
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_ROTATION_PITCH"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_ROTATION_YAW"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPAWN_ROTATION_ROLL"));
    } else {
        SP_ASSERT(false);
    }
    FActorSpawnParameters static_mesh_actor_spawn_parameters;
    static_mesh_actor_spawn_parameters.Name = Unreal::toFName(Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE_ACTOR_NAME"));
    ...
    FVector scale =
        FVector(
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE.MESH_SCALE_X"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE.MESH_SCALE_Y"),
            Config::get<double>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE.MESH_SCALE_Z"));
    static_mesh_actor_->SetActorScale3D(scale);

    // Load mesh and material
    auto sphere_mesh = LoadObject<UStaticMesh>(nullptr, *Unreal::toFString(Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE.STATIC_MESH")));
    SP_ASSERT(sphere_mesh);
    auto sphere_material = LoadObject<UMaterial>(nullptr, *Unreal::toFString(Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.SPHERE.MATERIAL")));
    ...
    camera_actor_spawn_parameters.Name = Unreal::toFName(Config::get<std::string>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA_ACTOR_NAME"));
    ...
    camera_actor_->GetCameraComponent()->FieldOfView =
        Config::get<float>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.FOV");
    camera_actor_->GetCameraComponent()->AspectRatio =
        Config::get<float>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.IMAGE_WIDTH") /
        Config::get<float>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.IMAGE_HEIGHT");
    ...
    auto observation_components = Config::get<std::vector<std::string>>("SP_SERVICES.LEGACY.SPHERE_AGENT.OBSERVATION_COMPONENTS");

    if (Std::contains(observation_components, "camera")) {
        camera_sensor_ = std::make_unique<CameraSensor>(
            camera_actor_->GetCameraComponent(),
            Config::get<std::vector<std::string>>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.RENDER_PASSES"),
            Config::get<unsigned int>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.IMAGE_WIDTH"),
            Config::get<unsigned int>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.IMAGE_HEIGHT"),
            Config::get<float>("SP_SERVICES.LEGACY.SPHERE_AGENT.CAMERA.FOV"));
...
std::map<std::string, ArrayDesc> SphereAgent::getActionSpace() const
{
    ...
    auto action_components = Config::get<std::vector<std::string>>("SP_SERVICES.LEGACY.SPHERE_AGENT.ACTION_COMPONENTS");
...
std::map<std::string, ArrayDesc> SphereAgent::getStepInfoSpace() const
{
    ...
    auto step_info_components = Config::get<std::vector<std::string>>("SP_SERVICES.LEGACY.SPHERE_AGENT.STEP_INFO_COMPONENTS");
