...
public:
    ...
    // typically called from a worker thread in the "engine_service.initialize" entry point to reset the work
    // queue's state, which is useful if the work queue is in an error state.
    void initialize();
