...
struct FSpRHIMemoryStats
{
...
#if BOOST_OS_WINDOWS
    // Fills stats from the given DXGI adapter, shared by our D3D11 and D3D12 code paths. Delegates directly to
    // UE::DXGIUtilities::GetD3DMemoryStats(...), see Engine/Source/Runtime/RHICore/Private/DXGIUtilities.cpp, which
    // implements the same logic in UE 5.5 and UE 5.8.
    static FSpRHIMemoryStats getD3DMemoryStats(IDXGIAdapter* dxgi_adapter)
    {
    ...
    // Engine/Source/Runtime/VulkanRHI/Private/VulkanMemory.cpp. FDeviceMemoryManager itself lives under
    // VulkanRHI/Private, with no public equivalent available in UE 5.5 (UE 5.8 added a public
    // IVulkanMemoryAllocator::GetMemoryStats() wrapper, obtained via RHIGetVulkanMemoryAllocator(), that
    // delegates to the exact same function, but that wrapper doesn't exist in UE 5.5), so we reimplement
    // the underlying Vulkan queries directly instead of delegating.
    static FSpRHIMemoryStats getVulkanMemoryStats()
    {
        FSpRHIMemoryStats stats;
        ...
        // Mimics UE 5.8's GVulkanBudgetPercentageScale, which scales each heap's reported budget by a
        // percentage before use (see Engine/Source/Runtime/VulkanRHI/Private/VulkanMemory.cpp). It defaults
        // to 100 (a no-op) and is only exposed as the console variable r.Vulkan.BudgetScale, not a linkable
        // symbol, so we read it the same way any other external code would have to.
        int32 vulkan_budget_percentage_scale = 100;
        if (IConsoleVariable* console_variable = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Vulkan.BudgetScale"))) {
            vulkan_budget_percentage_scale = console_variable->GetInt();
        }
        ...
        for (uint32 i = 0; i < vk_physical_device_memory_properties.memoryHeapCount; i++) {
            uint64 heap_budget = vulkan_budget_percentage_scale * vk_physical_device_memory_budget_properties.heapBudget[i] / 100;
            uint64 heap_usage = vk_physical_device_memory_budget_properties.heapUsage[i];

            if (vk_physical_device_memory_properties.memoryHeaps[i].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
                stats.BudgetLocal += heap_budget;
                stats.UsedLocal += heap_usage;
            } else {
                stats.BudgetSystem += heap_budget;
                stats.UsedSystem += heap_usage;
            }
        }
...
public:
    ...
    static FSpRHIMemoryStats RHIGetMemoryStats()
    {
        ...
        if (interface_type == ERHIInterfaceType::D3D12) {
                ...
                // ID3D12DynamicRHI::RHIGetAdapterDescs() always returns exactly one entry, see FD3D12DynamicRHIModule::FindAdapter(...)
                // in Engine/Source/Runtime/D3D12RHI/Private/Windows/WindowsD3D12Device.cpp
                ...
                stats = getD3DMemoryStats(dxgi_adapter);
            #else
                SP_ASSERT(false);
            #endif
