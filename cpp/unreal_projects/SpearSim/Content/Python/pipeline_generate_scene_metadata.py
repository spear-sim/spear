...
def get_actor_desc(actor):
    ...
    actor_class = actor.get_class().get_full_name()
...
def get_component_desc(component):
    component_class = component.get_class().get_full_name()
...
def get_editor_property_descs(uobject, ignore=[]):
    ...
    # so we use the fact that all editor properties are exposed as Python attributes, and iterate over
    ...
    # that the attribute is a valid editor property. For each editor property, if the value of the editor
    # property is itself a uobject, then we recurse, otherwise we store the value in our output dict.
    editor_property_descs = {}
    attribute_names = [ attribute_name for attribute_name in sorted(dir(uobject)) if attribute_name not in ignore ]
    for attribute_name in attribute_names:
        attribute_is_editor_property = False
        try:
            editor_property = uobject.get_editor_property(attribute_name)
            attribute_is_editor_property = True
        except:
            pass
        if attribute_is_editor_property:
            editor_property_descs[attribute_name] = get_editor_property_desc(editor_property)
    return editor_property_descs


def get_editor_property_desc(editor_property):

    # If the editor property is an Unreal object, then recurse via get_editor_property_descs(...) and return
    # a dict of the form: { "class": ..., "editor_properties": ... }
    if isinstance(editor_property, unreal.Object):
        editor_property_class = editor_property.get_class().get_full_name()
