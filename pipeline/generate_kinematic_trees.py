...
def process_scene():
    ...
    actors = { actor_name: actor_kinematic_tree for actor_name, actor_kinematic_tree in actors.items() if actor_kinematic_tree["root_node"]["has_valid_geometry"] }
    ...
    spear.log("Generating JSON file: " + kinematic_trees_actors_json_file)
...
def get_kinematic_tree_node(actor_name, component_desc, component_is_root_within_node, transform_node_from_parent_component, log_prefix_str):
    ...
    if component_desc["class"] in scene_component_classes and not spear.pipeline.any_component_transform_absolute(component_desc):
        ...
        if component_desc["class"] in static_mesh_component_classes:
            ...
            if static_mesh_desc is not None:
                ...
                if static_mesh_asset_path.parts[:4] == ("/", "Game", "Scenes", args.scene_id):
                    ...
                    kinematic_tree_node["has_valid_geometry"] = True
        ...
        for child_component_name, child_component_desc in component_desc["children_components"].items():
            ...
            if child_component_kinematic_tree_node["has_valid_geometry"]:
                kinematic_tree_node["has_valid_geometry"] = True
