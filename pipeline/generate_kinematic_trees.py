...
def get_kinematic_tree(actor_desc):
    ...
    return {
        "root_node": get_kinematic_tree_node(
            ...
            transform_node_from_parent_component=spear.utils.pipeline_utils.identity_transform,
            log_prefix_str="    ")}
...
def get_kinematic_tree_node(actor_name, component_desc, component_is_root_within_node, transform_node_from_parent_component, log_prefix_str):
    ...
    kinematic_tree_node = {
        ...
        "transform_parent_node_from_current_node": spear.utils.pipeline_utils.get_transform_data_from_transform(transform=spear.utils.pipeline_utils.identity_transform)}
    ...
    if component_desc["class"] in scene_component_classes and not spear.utils.pipeline_utils.any_component_transform_absolute(component_desc=component_desc):

        transform_node_from_current_component = \
            spear.utils.pipeline_utils.compose_transform_with_component(
        ...
        if component_is_root_within_node:
            ...
            transform_node_from_current_component = spear.utils.pipeline_utils.identity_transform
        ...
        physics_constraint_components = { name: desc for name, desc in physics_constraint_components.items() if not spear.utils.pipeline_utils.any_component_transform_absolute(component_desc=desc) }
            ...
            if child_component_kinematic_tree_node is not None:
                ...
                if child_component_is_root_within_node:
                    ...
                    physics_constraint_component_desc["pipeline_info"]["generate_kinematic_trees"]["transform_current_node_from_current_component"] = \
                        spear.utils.pipeline_utils.get_transform_data_from_transform(transform_node_from_child_physics_constraint_component)
