...
def find_actors(actor_class=None):
    ...
    actors = sorted(actors, key=lambda actor: get_stable_name_for_actor(actor))
...
def find_actor(name):
    actors = [ actor for actor in find_actors() if get_stable_name_for_actor(actor) == name ]
    if len(actors) == 1:
        return actors[0]
    else:
        return None
...
def find_components(actor, component_class=unreal.ActorComponent):
    ...
    if actor.root_component is not None:
        ...
        candidate_components = [ c for c in candidate_components if get_stable_name_for_component(c) not in component_names ]
        ...
        component_names = component_names + [ get_stable_name_for_component(c) for c in candidate_components ]
...
def get_stable_name_for_component(component, include_stable_actor_name=False):
    if include_stable_actor_name:
        actor_name_str = get_stable_name_for_actor(component.get_owner()) + ":"
    else:
        actor_name_str = ""
