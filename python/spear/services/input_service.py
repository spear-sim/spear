...
class InputService():
    ...
    def inject_key_for_actor(self, actor, chord, key_event):
        self._entry_point_caller.call_on_game_thread("input_service.inject_key_for_actor", actor, spear.utils.func_utils.to_json_string(obj=chord), key_event)

    def inject_touch_for_actor(self, actor, key_event, finger_index, location):
        self._entry_point_caller.call_on_game_thread("input_service.inject_touch_for_actor", actor, key_event, finger_index, spear.utils.func_utils.to_json_string(obj=location))
    ...
    def inject_vector_axis_for_actor(self, actor, vector_axis_name, vector_axis_value):
        self._entry_point_caller.call_on_game_thread("input_service.inject_vector_axis_for_actor", actor, vector_axis_name, spear.utils.func_utils.to_json_string(obj=vector_axis_value))
