...
int main(int argc, const char **argv)
{
    ...
    if (argc < 2)
    {
        ...
        printf("-i <obj/stl>            : Import mesh as a wavefront OBJ file or as a STL file.\n");
    ...
    else
    {
        ...
        bool useWavefrontInputFormat = true;
        uint32_t tcount = 0;

        // Mesh import structures
        stl_reader::StlMesh<double, unsigned int> stl;
        WavefrontObj w;
        ...
        for (int i = 2; i < argc; i += 2)
        {
            ...
            else if (strcmp(option, "-i") == 0)
            {
                ...
                {
                    useWavefrontInputFormat = true;
                    printf("Importing mesh as a wavefront OBJ file.\n");
                }
                else if (strcmp(value, "stl") == 0)
                {
                    useWavefrontInputFormat = false;
                    printf("Importing mesh as an ASCII or binary STL file.\n");
                }
                else
                {
        ...
        if (useWavefrontInputFormat)
        {
            tcount = w.loadObj(inputFile);
        }
        else
        {
            stl.read_file(inputFile);
            tcount = stl.num_tris();
        }
        ...
        if (useWavefrontInputFormat)
        {
            points = new double[w.mVertexCount * 3];
        ...
        {
            points = new double[stl.num_tris() * 3];
            const double *c = stl.raw_coords();
            for (uint32_t i = 0; i < stl.num_vrts() * 3; i++)
            {
                points[i] = c[i];
            }
        }
        ...
        bool canceled = false;
        {
            ...
            {
                iface->Compute(points, stl.num_vrts(), stl.raw_tris(), stl.num_tris(), p);
            }
            while (!iface->IsReady())
            {
                ...
                if (dot && format != ExportFormat::NONE)
                {
                    ...
                    else if (format == ExportFormat::STL)
                    {
                        for (uint32_t i = 0; i < iface->GetNConvexHulls(); i++)
                        {
                            ...
                            char hullName[2048];
                            if (ch.m_volume > min_volume)
                            {
                                snprintf(hullName, sizeof(hullName), "%s%03d.stl", baseName.c_str(), i);
                            }
                            else
                            {
                                snprintf(hullName, sizeof(hullName), "VolumeError_%s%03d.stl", baseName.c_str(), i);
                            }
                    ...
                    else if (format == ExportFormat::WAVEFRONT)
                    {
                        for (uint32_t i = 0; i < iface->GetNConvexHulls(); i++)
                        {
                            ...
                            {
                                snprintf(outputName, sizeof(outputName), "VolumeError_%s%03d.obj", baseName.c_str(), i);
                            }
