...
- When adding a new custom struct type to the RPC interface, plumb it through all layers: `SpTypes.h` (C++ struct), `MsgpackAdaptors.h` (serialization), `FuncSignatureRegistry.h` (type ID + type desc), `python_ext/cpp/types.h` (client struct), `python_ext/cpp/msgpack_adaptors.h` (client serialization), `python_ext/cpp/func_signature_registry.h` (client type ID + type desc + getReturnValue), `python_ext/cpp/spear_ext.cpp` (nanobind class + entry point signatures), and `editor_utils.py` (Python ClientStruct + deserialization). Only add the serialization direction that is actually needed (e.g., `object_with_zone` only if the server sends it, `convert` only if the server receives it).
...
- In `spear_ext.cpp`, entry point signatures are grouped by argument count (0 args, 1 args, etc.) and sorted lexicographically within each group by the tuple of type IDs `(return_type_id, arg1_id, arg2_id, ...)`. The type ID ordering is defined in the func signature registry files. When adding a new signature, place it in the correct position within its arg-count group according to this ordering.
- When refactoring RPC entry point signatures (e.g., adding a parameter), add the new signatures to `spear_ext.cpp` but do not try to identify and remove orphaned old signatures manually. Instead, rely on the runtime validation step at startup — it will report both missing signatures and orphaned signatures. Fix reported issues iteratively.
